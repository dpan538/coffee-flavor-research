...
[阶段 2] 第一份风味描述：3 + 5 = 8 个词（`describe()`，只取 7 个候选维度，名额按 V_target 的维度权重分配、词表按杯子轮换起点，每个词可归因到一个维度；醇厚度、烘烤苦感、发酵酒香、香料、瑕疵是评价维度，只进风味卡的评价行 `evaluate()`，2026-09-17）
...
- 机器阅读层重写（R3-D51，2026-09-21）：有助手把 flavorwords.com 说成「英语词汇学习网站」，搜索引擎的摘要猜它是闲置域名——原因是标题里没有「咖啡」、正文对不跑 JavaScript 的读取器是空的、事实已过时。现在：标题与描述用中英文写明「咖啡品鉴应用」；应用根节点里放一段真实的双语说明（应用启动后被替换，首屏只有字标与标语）；JSON-LD 图（WebSite / Person / WebApplication / FAQPage，其中一问明确回答「不是背单词网站」）；不需要 JavaScript 的静态页 `/about`；`llms.txt` 按现状重写并写明它是什么、不是什么；`robots.txt` 放行全部爬虫并点名各家助手与 AI 搜索；`sitemap.xml`；1200 × 630 分享图 `og.png`（`apps/pwa/scripts/build-og-image.mjs`）；manifest 描述；Service Worker 的导航回退放行 `/about` 与各文本文件；`vercel.json` 的响应头。这些文件里的事实是手写的，产品变化时要与 README 同步。
- SEO / SMO 与分语言地址（R3-D52，2026-09-21）：owner 在 Search Console 请求收录 `/about` 被拒——原因是 R3-D51 尚未部署，线上该地址是 404，部署后需重新请求。同一轮补齐：一种语言一个地址——`/zh`（lang zh-CN）与 `/en`（lang en）两张静态页由 `apps/pwa/scripts/build-static-pages.mjs` 生成（字标、五个图形、应用同款字体），三个页面与 sitemap 里的 hreflang 互指且一致，`/` 是 x-default；`/about` 转到 `/en`；响应头带 Content-Language。两页上的「打开 flavorwords」经 `/?lang=` 打开对应语言的应用：应用把它当作一次手动切换（记住），并把参数从地址栏去掉，规范地址仍只有 `/`；应用启动或切换语言时，标题与描述随语言更换（标签页、历史记录、分享面板、书签读到的就是读者的语言）。品牌与分享：`favicon.ico`（16/32/48，方形裁到字标）、Safari 固定标签的 mask-icon、`wordmark.svg`，均取自 `icon.svg`；Open Graph、Twitter 之外加 itemprop 三项（QQ、QQ 空间与部分微信版本的分享卡）；JSON-LD 增加 Brand（logo、slogan、sameAs 仓库）与两张语言页；品牌化的 404 页。搜索引擎：百度 / 360 / 搜狗的 applicable-device、no-transform、no-siteapp、renderer，一行关键词，robots.txt 点名更多爬虫；Apple 的 web-app metas。MiSans 子集把静态页的用字算进去（原先缺 46 个字，中文页会混排两种字体）。`tests/machine-readable-layer.test.ts` 守住容易悄悄坏掉的部分：hreflang 集合、canonical、页面语言、结构化数据、sitemap、数字与数据包一致。未做：各平台的站点验证码（需 owner 自己的账号）、IndexNow 主动推送。
...
**Safari 底部安全区、海报高度、添加到主屏幕（R3-D25）**

- 抽屉不再用纸色填底部安全区，改由每一页自己的内边距承担；文档画布颜色跟随底边所在的页面（首页与流程为色带色，抽屉内为深色或纸色），iOS Safari 工具栏后面与过滚动区域不再出现截断内容的白条。
- 两张海报改为图表吃掉文字之外的剩余高度（flex 列，SVG 以剩余高度与列宽为界），660px 与 900px 高度下都在面板内收尾。
- 语言切换左侧加入「添加到桌面」图标，覆盖苹果、安卓、鸿蒙三端：Chrome / Edge 直接弹浏览器安装提示；iOS / iPadOS 的 Safari 没有接口，弹出说明「分享 → 添加到主屏幕」；鸿蒙（HarmonyOS 4 与 NEXT 的华为浏览器）说明「右下角菜单 → 添加到桌面」；微信内置浏览器无法安装，提示「在浏览器打开」；已作为独立应用运行时隐藏。说明里的菜单名不拆行。文案在 appShell 的 install\* 字段。

**README 重写为数据产品研发记录（R3-D26）**

- 根 README 不再讲早期 atlas 原型：问题、中文摘要、产品流程、数据（8 个来源族、9,128 条有效记录、8,142 条可用向量、83,031 条断言、94 概念 × 12 维、主轴分布、16 组命名参考风味、文献登记状态、GACTT 聚合、权利姿态）、方法（V_pred / V_user / ΔV 与 α、一致性阈值、实测的 Q6 门槛、动态题面、证据标注、四项充分性测量）、工程、贡献声明、研究历程、现状与边界。每个新数字带 claim 标记并登记证据路径；公开状态源把 PWA 记为 IMPLEMENTED，阶段 PRODUCT_VECTOR_V1_PWA_LIVE；六张 PWA 截图替换四张 atlas 截图。

**owner 2026-09-16 / 17（R3-D28、R3-D29）**

- 候选词从产品图两批扩到 74 个具体对象；`describe()` 只取 7 个候选维度，8 个名额按 V_target 的维度权重分配（前 3 个维度各至少 1 个，其余按权重摊给达到最强维度 12% 的维度，词表 3 个词的维度最多 1 个），每个维度的词表从与 V_target 相关的位置开始取，先放每个维度的第 1 个词再放第 2 个；勾选按维度计。醇厚度、烘烤苦感、发酵酒香、香料、瑕疵是评价维度（`presentation.dimension_roles`）：不进候选词，最强的至多三个以「维度标签 + 一个词」出现在风味卡的评价行（`evaluate()`，卡片与导出 PNG 都有）。
- 升阶门控在候选子空间里比较：勾选向量与 V_user、V_pred 的余弦都先把评价维度清零再归一（阈值与 0.08 边距不变）。穷举 18,432 条：按顺序勾前 5 个不再开 Q6（它几乎覆盖了读者能勾的全部维度，不构成偏向），偏向自己答案的 5 个词 70.1%（Path 2/3/4 = 77% / 97% / 90%），随机 5 个 14.3%，Path 1 为 0。
- 第一题加「美式」（浓缩加水，借意式行）与「奶咖」（浓缩加奶，意式行在甜感与醇厚度各加 0.1 后归一，标 ESPRESSO_PROXY_PLUS_MILK_BIAS）；「挂耳」原并入手冲，owner 随后纠正为独立选项（R3-D32：客群不同，合并显得不专业；借杯测行，花香与果香各 −0.05），共十二个选项。
- 应用壳：字标在流程中点击回首页；流程中 EN 左侧的添加到桌面图标换成退出；离开后的首页有三块（开始 · 继续 · 关于），继续回到离开时的那一步，开始或首页清掉。

**owner 2026-09-17，第二轮（R3-D30）**
...
**owner 2026-09-17，第四轮（R3-D32）**

- 挂耳独立成选项（同手冲的杯测参考行，花香、果香各 −0.05 后归一），手冲标签恢复；做法共十二个。
- CI 的数据库过滤排除产品层路径（`db/data/product-vector-v1/**`、账本、`build-matrix-k-v1.py`、`export-frontend-copy.py`）：只改按键、标签或词表的推送只跑 web job。

**owner 2026-09-18（R3-D33、R3-D34）**

- 回改答案：进度条每段是 40 × 44 px 的点击区（条本身仍 4 px），已答的段与堆叠里的题目卡都能重开那道题；只改重开的那一题，其余答案保留并由引擎按新流程回放（`reopenQuestion` / `answerAndReplay` / `resumeKept`），不弹窗；新路径多出的题才会停下来问，不再需要的答案丢弃；来时那一题的段可原样返回。描述页与终卡上同样可回到题目，词、勾选与卡片随新结果重建。枚举测试要求任何回改后的会话与「用最终答案新开的会话」逐字段相同，穷举模式零差异（单改 52,473、跳转后改 91,884、连改两题 900,519、出结果后回改 375,318）。
- 关于这段描述：初始参考改为对照全部 6,516 条评审记录的解读（某类咖啡提到某种风味的比例 vs 整体），不再复述选项，固定的免责声明句删除；资料统计围绕确认的词，取最能说明该维度的选项或两两组合，分「支持 / 相当 / 与常态不同」三种说法；你的描述说明回答抬高和压低了什么、卡上哪些词来自这一偏向。句式中英各三种、位置对应，种子与语言无关，同一张卡两种语言数字一致，换一次访问可能抽到不同的统计。
- 首页图形每次出现重新抽取，只用五个简单图形（半圆、三角、圆、四分之一圆、菱形），变化来自顺序、朝向与配色。
- 更新机制补强：全局监听 controllerchange（另一个页签或普通刷新也会让新 worker 接管旧页面），并用不进缓存的 `version.json` 与运行中的构建标识对照，回到首页时刷新，带防循环保护。

**owner 2026-09-19（R3-D35）**

- 英文文案：题面与选项改为口语化的句子大小写（不再全大写），按钮字号与中文接近（三块并排时中文 22 px、英文 20 px）；开始入口写作 Start with this cup，题卡小标签 This cup。
- 终卡：保持接近正方形；风味词一行多个（英文首字母大写），评价以「维度 + 词」成对排在一行；去掉「已确认」字样（模型仍保留该状态）。一词一行的加长版试过，被否决。
- 导出 PNG：1200 × 2500（1.2 : 2.5），首页白底、黑字、细线框；层级靠字号、留白与线条——字标、序号与日期；风味词（最大）；评价；这杯咖啡；一段说明（放不下时按整句截断）；参考风味、网址与 slogan。不写「风味卡」字样；图形只保留一个简单图形，取第一确认维度的颜色，种子与语言无关（中英文同一图形与序号）。
- 香料词表顺序改为 肉桂｜丁香｜小豆蔻：评价行与 Q6 弹层显示肉桂；说明文字用 `dimension_gloss` 把维度写成具体的词，而不是「香料更突出」。待 owner 决定：中文维度名是否改写、是否加入「八角」。
- 关于这段描述（R3-D36）：资料统计只出一条，取偏离最大的那个统计（另一次到访只在强度达到最强者 60% 的统计里轮换）；先用平常话给结论，再用频率给数字（「评审记录里的 177 款中，每 10 款约 5 款写到花香，而所有咖啡平均约 3 款」），不再出现百分比与「整体」；初始参考只说方向、不带数字，方向相同的选项合并成一句。
- 导出卡改为邮票式（R3-D37）：放在终页深蓝（#1F3B5C）上，四周齿孔边；上半到风味词为止是白底黑字，下半是更深一档的蓝（#14283F）配白字；去掉上下的粗线与方框、去掉「评价 / 这杯咖啡 / 你的描述」小标题；字标缩小，右上角改为一行五个小图形（颜色取确认词的维度，与屏幕上的卡一致）；参考风味并入评价各行；卡上不出现「你」，说明改为一句第三人称的话（`cardNote`）；标语一行放下；中文风味词缩小 10%（112 px），英文取 84 / 80 / 76 / 72 px 中能排进三行的最大字号。
- 导出卡定稿的布局（R3-D38）：背景透明、齿孔从图像里挖掉，图片本身就是一张邮票，下半用终页的深蓝 #1F3B5C；补充描述两句（这一杯相对同类参考风味偏向什么、卡上哪些词来自这一偏向），第三人称；布局固定且中英文一致——补充描述与标语作为一组贴底，各行贴在白色部分下方、行距在 92–112 px 内伸缩，使行与补充描述之间保持约 150–190 px；白色部分只有两个高度：行数 ≥ 7 时 920（风味词流式、至多三行），行数 ≤ 6 时 1180（风味词一行一个）；英文风味词用 Fraunces 300（display 光学尺寸、字距 −0.02 em），其余英文不变；过长的值缩小字号而不压到标签。
- 导出卡的颜色与字重（R3-D39）：上半由纯白改为极浅的灰 #F1F1EF（透明图在白色屏幕上不会消失）；下半从应用已有的、能承载白字的四种颜色里按卡片种子取一种——藏蓝 #1F3B5C、森林绿 #2F7A4C、酒红 #8C4A4C、墨黑 #1E1C1A（与白字对比度 5.2–17.0），同一张卡中英文、每次导出颜色一致；绿与酒红底上 taste 五个字母改用调色板里的浅色；英文风味词字重 300 → 400。
- 动态题库方向获批（R3-D40）、第 3 批词入表（R3-D41）：每题最多 5 个选项，语料只剪枝不排序，第五个选项写清楚「不突出」，同一选项可有几种说法；新增「香气有多明显」「咽下去之后的余韵」两道评价题，苦感五档；20 个语料支持的具体词加入词表（74 → 94，「姜」写作「生姜」），详见 `TERM_EXPANSION_BATCH_3.md`、`DYNAMIC_QUESTION_BANK_DRAFT.md`。题库本身尚未实现。
- 相近的词是软约束，不设上限（R3-D42）：owner 问「词卡里会不会出现高度相似的词」。实测（18,432 张卡）：同一维度最多 4 个词（0.3%），99.7% 的卡不超过 3 个——维度层面由权重分配保证；但同一维度内的近义词原先没有任何机制，60% 的卡至少有一对（词表里相近的词挨着放，轮换又是连续取词）。owner 决定不设硬上限（有些咖啡确实围绕同一类风味塑造），改为软约束：词表加子类列（`tag_families`），`rotatedWordList` 先按子类交错排出基础顺序再按种子轮换，取词时各子类的第一个词先于任何子类的第二个词；名额多于子类时允许重复；读者的回答指向某个子类（`describe(result, locale, focus)`，供第二级问题使用）时该子类领先并可重复。近义词同现的卡从 60% 降到 25%（剩下的主要是跨维度的，如甜橙与橘子糖），不同词数与组合数不变。
- 动态题库第一阶段与引擎的纯函数（R3-D43）：题库源文件 `DYNAMIC_QUESTION_BANK.tsv`（8 题 121 个选项，含中英文的几种说法、对 12 维的加权、指向的词子类、语料概念）；`build-dynamic-question-bank.py` 只读公开文件，为 68 类咖啡（烘焙度段 × 处理法 × 豆种 × 是否意式，至少 30 条记录，最具体的优先）把每个选项池剪到 4 个并按选项池顺序存放，来源校正取半程；第二级的词与选项标签里的举例按该类咖啡的语料排序；Aroma / Aftertaste 分位只在同一烘焙度段内计算，单独成文件 `COFFEEREVIEW_AROMA_AFTERTASTE_AXES.tsv`。引擎 `dynamicBank.ts`：`contextGroup` / `shownQuestion` / `secondLevelQuestion` / `dynamicEffects`，全是（语境，答案，种子）的纯函数。口感一题不按语料剪枝——评审的 Body 打分同样是质量分（浅烘反而高），改按冲煮方式给窗口。会话与界面尚未接入，线上仍是六道固定题。
- 动态题库接入会话与界面（R3-D44）：会话加 `dynamic` 模式（应用使用；`fixed` 保留为一致性判断树的参考实现与穷举测试）。Q0–Q3 换成核心四题（酸与水果、香气主调、甜、口感），选项由 `shownQuestion` 给出：至多 5 个、按选项池顺序、第五个写清楚「不突出」、下方一行小字承接答不上来的读者；题面沿用「上一题答案 + 之后，」的承接语。追问至多 2 道（`followUpKeys`）：答案严重矛盾时问苦感与整体印象，轻度分歧时问整体印象与一道细节题，一致时问两道细节题（第二级「更像哪一个」、苦感、余韵、香气；深烘或烘烤类答案先问苦感）；读者有两道核心题答不上来时只追问 1 道。引擎同时认字母与新选项 id（`slotVector`），读者向量由新选项自带的权重求和；第二级选中的词领头、第一级指向的子类其次（`wordHints` → `describe`）；苦感、香气、余韵的回答进评价行（至多 4 行）；余韵或香气答得明确且该类咖啡在同烘焙度内确实更常得高分时，那一条资料统计可以换成同烘焙度内的对照。回改按提问顺序保留答案，穷举对照「改后 ≡ 用最终答案新开一局」；旧的固定题存档不再恢复。实测 15,000 条序列：Path 1/2/3/4 = 26.0 / 65.9 / 3.6 / 4.4%，Q6 触发率前 5 个 0.0%、偏向自己 62.7%，第二级的词 100% 出现在候选词里。
- 新旧流程的模拟对照（R3-D45 → R3-D46，`QUESTION_FLOW_SIMULATION.md`）：500 杯同来源（CoffeeReview）+ 500 杯换来源（Cup of Excellence），剪枝表留出这 1,000 条后重建，两套流程用同一条作答规则。提升在词上，而且换来源也成立——终卡的词落在真实提到的子类里 33.5% → 62.2%（区间 +26.7 到 +30.5），换来源 71.6% → 86.5%；精确到词 10.1% → 27.4%，换来源 34.9% → 64.8%。12 维向量与参考风味这一层没有可靠的提升（同来源余弦 0.847 → 0.835，参考风味排第一 +3.2 不显著），不对外宣称。描述词很多的杯子上召回下降（换来源 43.7% → 37.5%）：卡上的词向读者指到的类别集中，是 R3-D42 的取舍。代价是平均多问 0.6–0.8 题。第一轮 200 杯里「参考风味排第一 21.3% → 32.7%」已更正——它有一部分来自模拟读者不对称的作答规则。
- 减轻过拟合（R3-D46）：留出；换来源验证；小样本分组向上一级收缩（`SHRINK = 60`，n 条记录的分组保留 n / (n + 60) 的自身估计；68 组里 27 组保留的选项因此变化，指标无可测变化，为稳定而采用）；选项权重不由语料学习（学出来的与手工的相当，少一层拟合）。
- 「更重的计算」（R3-D46 → R3-D47）：选项权重由语料学习——无提升，不采用；近邻检索决定全部候选词——精确度掉 17 个点，不采用；只补读者没给信号的词位、按近邻里出现多少排序——命中率最高，但甜橙出现在 75% 的穷举卡片上，弃用。
- 近邻补全（R3-D47，`neighbours.ts` + `build-neighbour-index.py` → `product-vector-v1-neighbours.json`）：owner 的原则是强化「本身明显但有波动」的候选，不强化本来就强的候选。读者没给信号的维度里，取与 V_target 最近的 60 条记录（相似度四次方加权）；一个概念只有在近邻里至少 10% 提到、且比率至少是全部记录的 2 倍时才计分，计分的词领先，其余保持轮换，子类分散照旧；读者的回答（子类或第二级的词）永远优先；六道固定题不启用，逐位不变。索引每条记录只有 1 字节量化的 12 维向量与概念编号（182 KB，gzip 约 90 KB），不含 id、名称、来源、文字、分数。成对对照：同来源 +1.4 / +3.2 / +2.6（均显著），换来源 +0.2（不显著）/ +2.0 / +1.4；有效词数 62.5 → 59.9。V_target 的纯函数，所以回改、重放、双语一致。
- 第 4 批术语：木质调（R3-D48，`TERM_EXPANSION_BATCH_4.md`）：柏木、橡木、檀香、松木、桦木、干燥树皮、沉香进入木质泥土维度，九种木头同属子类 `wood`；词表 94 → 101。风干雪松在穷举卡片上的占比 15.9% → 7.7%。烟熏、木炭留在苦感的评价语言里，木片、干草属于瑕疵，不进候选词。
- 双语对齐的修正（2026-09-20）：`describe` 里每个维度的名额上限原来按当前语言的词表长度计算，两种语言的消费者用语数量不同，约 0.3% 的卡片最后两个词的顺序因此不一致（勾选按位置跨语言映射）。上限改为统一按中文词表长度计算；中文结果逐位不变。
- 选项数量（R3-D49）：每题从候选里选出 4 个；只有最前面两题是 5 个，第五个是「说不上来」——正式的选项，不再是选项下方的一行小字。第一题：三个语料剪枝的选项 +「酸不突出」+「说不上来」；第二题：三个剪枝的选项 +「水果：果香最突出」+ 香气题自己的说不上来（「香气是一个整体，挑不出最突出的一类」）；甜与口感：三个选项 +「不突出 / 厚薄适中」；第二级：三个词 +「说不上来」；香气强度、整体印象：四级；苦感与余韵：按烘焙度显示五级中的四级（`scale_windows`），苦感只有作为强修正（前四题严重矛盾，`FlowStep.strong`）时显示完整五级。任何选项都不回指前面的题目（「还是水果为主（同上一题）」改为「水果：果香最突出」）。数量与窗口都在题库构建里（`tappable`、`scale_windows`），代价见 `QUESTION_FLOW_SIMULATION.md`。
- 再确认一步里选中的词必须上卡（R3-D49 后续）：那一步显示的是词（`q6Word`，每个维度词表的第一个词）。读者选中的每一个词——候选维度或评价维度都一样——原样成为终卡上的词，排在读者自己勾选的词之前，合计至多五个（超过五个时多出的评价词退回导出卡的评价行）；卡片保留读者自己勾选的词，不添加读者没选过的词（原来再确认之后直接换成第二次描述的前五个）；已经在词里的词不重复成评价行；保留的词切换语言时按维度词表逐位翻译（`localizeWord`）。屏幕上的终卡把评价行（醇厚度、烘烤苦感、香气、余韵）排成两栏细线表格，格式与上方的冲煮、烘焙等行一致（标签 + 值），四行只占两行高；整张卡收紧了内边距、行距与词的行高（844 px 手机上 373 → 324 px）。「香气 · 清楚」改为「香气 · 明显」。
- 勾选 3–5 个词（R3-D49 后续）：五个是建议不是条件，三个就能生成风味卡（`first_description.pick_min = 3`）；提示语「选出最贴近你感受的 3–5 个词，建议选 5 个。」
- 叠卡只留题目卡（R3-D49 后续）：语境设置阶段照旧收集语境卡；从第一道题开始，上方只保留题目卡（至多六张），每张露出至多 30 px 供手指点选；流程上下比例 30 : 70（740 px 以下的手机 28 : 72），选项高度 54–104 px。
- 资料说明不给数字（R3-D49 后续）：「评审记录里的 738 款中，每 10 款约 4 款写到木质泥土，而所有咖啡平均约 3 款」这类写法会让读者觉得信息被过度平均、怀疑过拟合。那一条资料说明现在只用平实的话比较「这类咖啡」与「一般咖啡」，不出现任何数字（记录条数、频率都不出现，初始参考里的那一处条数也去掉）；「更常 / 更少」要有至少 10 个点的差距才说（原来 5 个点），更接近时说「差不多」。
- 进度条不超过六格（R3-D49 后续）：回改使路径变化之后，旧路径里新路径不会再问的追问答案原来要等流程结束才丢弃，进度条把它们也画成一格（owner 看到 8 格）。现在四道核心题答完时流程给出这条路径的全部题目（`FlowStep.plannedKeys`），重放时立即丢弃不在其中的保留答案；测试对每一次回改断言「已答 + 当前 + 保留 ≤ 6」。「回到风味描述」不是一道题，画成圆点而不是第七格。
- 卡片跟随读者的信号（R3-D49，`dynamicBank.wordHints` → `describe`）：水果一题里读者看过没选的种类排到该维度的最后（不排除、不设上限；答不上来的读者不算看过没选）；读者答了柑橘（指向整个酸质维度、同时带果香权重），果香的名额由柑橘词填，词保留自己的维度并记下所占的名额（`Word.slot`），勾选时两个维度都算确认。量过而未采用：所有题目都按「看过没选」处理（−4.0 / −6.1）；完全没有读者信号的维度不出词（再确认一步对按顺序勾选的读者触发率 0.1% → 51.7%）。后者作为开关留在数据包里（`question_flow.first_description.prior_slots`，现值 `keep`）。
