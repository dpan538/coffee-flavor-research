...
describe("product-vector-v1 engine", () => {
  ...
  it("reports the basis of every context option and ignores options without a corpus row", () => {
    const { vPred, contextBasis } = buildVPred({
      c0_preparation: "cold_brew",
      c1_roast: "light",
    });
    expect(contextBasis.map((b) => b.basis)).toEqual([
      "NO_CORPUS_ROW_LITERATURE_CLAIM_PENDING",
      "CORPUS_MEASURED",
    ]);
  ...
  it("ranks profiles by cosine and keeps scores as similarities", () => {
    const result = infer(
      ...
      { Q5_acid: "A", Q8_aroma: "A", Q10_clean: "A" },
    );
    expect(result.profiles).toHaveLength(3);
    expect(result.profiles[0]!.similarity).toBeGreaterThanOrEqual(
      result.profiles[1]!.similarity,
    );
...
describe("product-vector-v1 presentation layer", () => {
  ...
  it("priority 2: dimension-level tags for a bare vector, from the owner's CN tag lists, without abstract dimension names", () => {
    ...
    expect(zh.some((t) => t.includes("floral") || t.includes("fruity"))).toBe(
      false,
    );
  });
  ...
  it("defect guard: no defect tag on a clean vector, defect tag when defect dominates", () => {
    ...
    expect(
      displayTags(bad, "zh-CN", 4, ["sensory.musty", "sensory.jasmine"]),
    ).toContain("霉味");
  });

  it("science fold: combination statements only when every part is answered, most specific first, with evidence state", () => {
    const lines = statementsFor(
      { c0_preparation: "pour_over_v60", c1_roast: "light" },
      "en",
    );
    ...
    expect(
      lines.every((l) =>
        ["CORPUS_MEASURED", "LITERATURE_CLAIM"].includes(l.evidenceState),
      ),
    ).toBe(true);
    expect(lines.some((l) => l.evidenceState === "OWNER_STATEMENT")).toBe(
      false,
    );
    expect(
      lines.every(
    ...
    expect(
      statementsFor({ c1_roast: "light" }, "en").map((l) => l.about),
    ).not.toContain("C0_POUR_OVER_V60__C1_LIGHT");
  });
  ...
  it("renders one vector backend in two languages: minimalist tags for zh-CN, scientific wording for en", () => {
    ...
    expect(
      zh.science.some((l) => l.about === "C0_POUR_OVER_V60__C1_LIGHT"),
    ).toBe(true);
    expect(en.science.every((l) => l.evidenceState && l.citationRef)).toBe(
      true,
    );
  });
});
...
describe("product-vector-v1 question flow (coherence decision tree)", async () => {
  const flow =
    await import("../packages/flavor-data/src/product-vector-v1/flow");
  const ctx = {
    c0_preparation: "pour_over_v60",
    c1_roast: "light",
    c2_process: "washed",
  ...
  it("path rhythm over every answer sequence of the bank (Q2 and Q4 carry a 'not noticeable' exit since the copy review): ~20% fast Path 1, ~50% corrective Path 2, ~25% severe Paths 3/4", () => {
    ...
    let total = 0;
    let evidenceTotal = 0;
    for (const q0 of opts("Q0"))
      for (const q1 of opts("Q1"))
        for (const q2 of opts("Q2"))
          for (const q3 of opts("Q3"))
            for (const q4 of opts("Q4"))
              for (const q5 of opts("Q5")) {
                ...
                total += 1;
                if (
                  !(["Q0", "Q1", "Q2"] as const).some((s) =>
                    flow.absentAnswer(full, s),
                  )
                ) {
                  withEvidence[String(step.path)] =
                    (withEvidence[String(step.path)] ?? 0) + 1;
                  evidenceTotal += 1;
                } else {
  ...
  it("measures coherence in profile-signature space, so consistent answers on different dimensions still agree", () => {
    ...
    expect(flow.coherence(bright, honeyFloral).similarity).toBeGreaterThan(
      flow.coherence(bright, caramelDark).similarity,
    );
  ...
  it("every complete answer set reaches delivery with a path and a coherence history", () => {
    ...
    for (const q0 of options)
      for (const q1 of options)
        for (const q2 of options)
          for (const q3 of options) {
            const answers: Record<string, string> = {
              Q0: q0,
              Q1: q1,
              Q2: q2,
  ...
  it("delivers a 3 + 5 description whose words are attributable to dimensions and distinct", () => {
    ...
    expect(
      zh.all.every(
        (w) => flow.dimensionCount === 12 && typeof w.dimension === "string",
      ),
    ).toBe(true);
    expect(zh.prompt).toContain("5");
    expect(flow.describe(result, "en").all.map((w) => w.text)).not.toEqual(
      zh.all.map((w) => w.text),
    );
  });
  ...
  it("escalation gate: Q6 only when a severe conflict happened and the picks side with the user against the theory", () => {
    ...
    const gateA = flow.escalationGate(
      calmStep,
      result,
      description.all.slice(0, 5),
    );
    ...
    const userSide = [
      { text: "x", dimension: "floral" },
      { text: "y", dimension: "floral" },
      { text: "z", dimension: "floral" },
      { text: "w", dimension: "acidity" },
      { text: "v", dimension: "sweetness" },
    ];
    const gateB = flow.escalationGate(
      severeStep,
      {
        ...result,
        vPred: flow.picksToVector([
          { text: "n", dimension: "nutty_chocolate" },
          { text: "b", dimension: "bitter_roasted" },
        ]),
      },
      userSide,
    );
  ...
  it("Q6 offers one word per most-disputed dimension and applies a strong correction", () => {
    ...
    expect(
      corrected.vUser[DIMENSIONS.indexOf("nutty_chocolate")],
    ).toBeGreaterThan(0);
  ...
  it("final card carries the user's own words, 1-2 attribution sentences plus the bias line (each with an evidence state), and the closing line", () => {
    ...
    expect(
      card.science.some((l) => l.evidenceState === "OWNER_STATEMENT"),
    ).toBe(false);
    const delta = card.science.find(
  ...
  it("the card's group name follows the confirmed words: picks that leave the suggested group retitle the card (copy review 2)", () => {
    ...
    const away: Array<{ text: string; dimension: string }> = [
      { text: "黑巧克力", dimension: "bitter_roasted" },
      { text: "烤榛果", dimension: "nutty_chocolate" },
      { text: "丝绒奶油", dimension: "body" },
      { text: "风干雪松", dimension: "woody_earthy" },
      { text: "烟熏可可", dimension: "bitter_roasted" },
    ];
  ...
  it("every question about a taste has a 'not noticeable' exit and an absence never reads as a severe conflict", () => {
    const bank = productVectorBundle.question_bank as Record<
      string,
      { options: Record<string, { label: Record<string, string> }> }
    >;
