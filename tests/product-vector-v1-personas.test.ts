...
function userSidePicks(
  result: ReturnType<typeof flow.inferFromFlow>,
  description: ReturnType<typeof flow.describe>,
) {
  return [...description.all]
    .sort(
      (a, b) =>
        (result.vUser[DIMENSIONS.indexOf(b.dimension)] ?? 0) -
        (result.vUser[DIMENSIONS.indexOf(a.dimension)] ?? 0),
    )
    .slice(0, 5);
}
...
describe("owner personas", () => {
  it("Persona A — light pour-over, jasmine and citrus, hates bitterness: Path 1 in 5 questions, no Q6, floral/acid profile", () => {
    ...
    expect([
      ...
      "Citrus & Yuzu Acidity",
      "Yellow Peach & Ripe Fruit",
      "Herbal & Green Tea",
    ]).toContain(top.owner_name.en);
  ...
  it("Persona B — chose dark espresso but wants bright peach acidity: context conflict → Path 2 correction, card, no Q6", () => {
    ...
    const persona: Persona = {
      name: "B",
      context: DARK,
      answers: { Q0: "A", Q1: "C", Q2: "C", Q3: "B", Q4: "B", Q5: "A" },
    };
    const { step, order, result, description } = run(persona);
    ...
    expect(step.escalationEligible).toBe(false);
    expect(
      flow.escalationGate(step, result, description.all.slice(0, 5)).escalate,
    ).toBe(false);
  });
  ...
  it("Persona C — '完全不酸、不要苦、要非常浓郁甜感': negation guard maps it; the answers are internally consistent, so no Q6", () => {
    ...
    const persona: Persona = {
      name: "C",
      context: MED,
      answers: { Q0: "C", Q1: "C", Q2: "C", Q3: "B", Q4: "B", Q5: "B" },
    };
  ...
  it("Persona C' — the sensory paradox (light-roast acidity and florals, then heavy dark bitterness): severe, and Q6 opens when the picks confirm the bias", () => {
    const persona: Persona = {
      name: "C'",
      context: LIGHT,
      answers: { Q0: "A", Q1: "A", Q2: "B", Q3: "C", Q4: "A", Q5: "B" },
    };
    const { step, order, result, description } = run(persona);
    ...
    const gate = flow.escalationGate(
      step,
      result,
      userSidePicks(result, description),
    );
    ...
    if (gate.escalate) {
      ...
      const corrected = flow.applyQ6(
        result,
        options.slice(0, 2).map((o) => o.dimension),
      );
...
describe("persona table: every persona delivers, records its path, and gets a coherent bilingual card", () => {
  const personas: Persona[] = [
    {
      name: "B2 fermented lover, soft acid",
      context: LIGHT,
      answers: { Q0: "B", Q1: "C", Q2: "C", Q3: "B", Q4: "B", Q5: "B" },
    },
    {
      name: "D classic nutty",
      context: MED,
      answers: { Q0: "C", Q1: "B", Q2: "B", Q3: "B", Q4: "A", Q5: "B" },
    },
    {
      name: "E dark heavy",
      context: DARK,
      answers: { Q0: "C", Q1: "B", Q2: "B", Q3: "C", Q4: "A", Q5: "B" },
    },
    {
      name: "J bright then chocolate",
      context: LIGHT,
      answers: { Q0: "A", Q1: "A", Q2: "B", Q3: "B", Q4: "A", Q5: "A" },
    },
    {
      name: "K acid hater, floral",
      context: LIGHT,
      answers: { Q0: "C", Q1: "A", Q2: "A", Q3: "A", Q4: "B", Q5: "A" },
    },
    {
      name: "H sweet smooth",
      context: MED,
      answers: { Q0: "C", Q1: "A", Q2: "A", Q3: "B", Q4: "B", Q5: "A" },
    },
    {
      name: "L winey dark",
      context: DARK,
      answers: { Q0: "B", Q1: "C", Q2: "B", Q3: "C", Q4: "A", Q5: "B" },
    },
    {
      name: "F jammy natural",
      context: LIGHT,
      answers: { Q0: "B", Q1: "C", Q2: "C", Q3: "B", Q4: "B", Q5: "B" },
    },
    {
      name: "I mixed signals",
      context: MED,
      answers: { Q0: "A", Q1: "B", Q2: "C", Q3: "A", Q4: "A", Q5: "B" },
    },
    {
      name: "G tea-like clean",
      context: LIGHT,
      answers: { Q0: "A", Q1: "A", Q2: "A", Q3: "A", Q4: "B", Q5: "A" },
    },
  ];
  const expected: Record<string, number> = {
    "D classic nutty": 1,
    "E dark heavy": 1,
    "J bright then chocolate": 3,
    "K acid hater, floral": 2,
    "B2 fermented lover, soft acid": 2,
    "G tea-like clean": 1,
  };
  ...
  for (const persona of personas) {
    it(persona.name, () => {
      ...
      if (expected[persona.name])
        expect(step.path).toBe(expected[persona.name]);
      ...
      expect(en.all.map((w) => w.dimension)).toEqual(
        description.all.map((w) => w.dimension),
      );
      const card = flow.finalCard(result, description.all.slice(0, 5), "zh-CN");
