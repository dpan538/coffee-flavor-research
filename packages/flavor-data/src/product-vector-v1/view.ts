...
export type ContextCard =
  ...
  | {
      ...
      multi: true;
      max: number;
      toggle: { single: string; blend: string };
      optional: false;
    }
  | {
      key: "c2_process";
...
const LABELS: Record<string, Record<Locale, string>> = {
  ...
  ethiopian_landrace: {
    "zh-CN": "埃塞原生种 Heirloom",
    en: "Ethiopian landrace",
  },
...
const TITLES: Record<string, Record<Locale, string>> = {
  ...
  c2_origin: {
    "zh-CN": "咖啡豆的产地（可选）",
    en: "Where the beans are from (optional)",
  },
};
...
function chips(axis: string, locale: Locale): Chip[] {
  return Object.entries(K[axis] ?? {}).map(([value, row]) => ({
    value,
...
export function contextCatalog(locale: Locale): ContextCard[] {
  const continents = ((
    rules as { origin_continents?: Record<string, Record<Locale, string>> }
  ).origin_continents ?? {}) as Record<string, Record<Locale, string>>;
  const origins = Object.entries(
    rules.origin_regions as Record<
      string,
      { label: Record<Locale, string>; bias: string[]; continent?: string }
    >,
  ).map(([value, r]) => ({
    value,
  ...
  const groups = Object.entries(continents).map(([key, label]) => ({
    key,
    label: label[locale],
  }));
  return [
    {
      key: "c0_preparation",
      title: TITLES.c0_preparation![locale],
      chips: chips("C0", locale),
      multi: false,
      optional: false,
    },
    {
      key: "c1_roast",
      title: TITLES.c1_roast![locale],
      chips: chips("C1", locale),
      multi: false,
      optional: false,
    },
    {
      ...
      toggle:
        locale === "zh-CN"
          ? { single: "单品 SOE", blend: "拼配 Blend" }
          : { single: "Single origin", blend: "Blend" },
      optional: false,
    },
    {
      key: "c2_process",
      title: TITLES.c2_process![locale],
      chips: chips("C2_process", locale),
      multi: false,
      optional: false,
    },
    {
...
export function normalizeContext(input: ContextAnswers): ContextAnswers {
  ...
  if (Array.isArray(out.c2_variety)) {
    const unique = [...new Set(out.c2_variety.filter(Boolean))].slice(
      0,
...
export type EscalationModalModel = {
  kind: "escalation";
  title: string;
  options: Array<{ dimension: string; text: string }>;
  submitLabel: string;
  reason: string;
};
export type ScreenModel =
  | QuizCardModel
  | { kind: "describe_ready" }
  | FirstDescriptionCardModel
  | EscalationModalModel
  | ResultCardModel;
...
export function screenModel(session: Session): ScreenModel {
  ...
  if (step.kind === "picks" && session.description) {
    return {
      ...
      heading: ((
        bundle.presentation as { preview_heading?: Record<Locale, string> }
      ).preview_heading ?? {
        "zh-CN": "这杯咖啡的风味",
        en: "This cup's flavor",
      })[locale],
    };
  }
  if (step.kind === "q6" && session.q6) {
    return {
      ...
      options: session.q6.options.map((o) => ({
  ...
  return {
    ...
    science: (card?.science ?? []).map((l) => ({
      text: l.text,
      evidenceState: l.evidenceState,
      citationRef: l.citationRef,
      sourceTitle: l.sourceTitle,
      sourceLicence: l.sourceLicence,
      label: l.label,
      citation: l.citation,
    })),
...
export function cupInfo(
  ...
  const names: Record<string, [string, string]> = {
    c0_preparation: ["冲煮", "Brew"],
    c1_roast: ["烘焙", "Roast"],
    c2_variety: ["豆种", "Variety"],
    c2_process: ["处理", "Process"],
    c2_origin: ["产地", "Origin"],
  };
  ...
  for (const key of [
    ...
    const text =
      key === "c2_origin"
        ...
        : values
            .map((v) =>
              contextLabel(v, locale) !== v
                ? shortContextLabel(v, locale)
                : label(v, locale),
            )
            .join(" + ");
...
export function q6Copy(locale: Locale): { title: string; submit: string } {
  return locale === "zh-CN"
    ? { title: "哪些描述更贴近你的感受？", submit: "更新风味卡" }
    : {
        title: "Which of these are closer to what you tasted?",
        submit: "Update my card",
      };
}
...
export function appShell(locale: Locale) {
  return locale === "zh-CN"
    ? {
        ...
        title: "风味，自有表达。",
        subtitle:
          "像柑橘，像可可，或是某种熟悉却一时叫不出名字的味道。flavorwords 帮你找到贴近感受的词，组成这一杯的风味卡。",
        lead: "",
    ...
    : {
        ...
        subtitle:
          "Some coffees are easy to taste, but harder to describe. Find the words for what you taste, and bring them together in your own flavor card.",
        lead: "",
        claim: "",
        start: "Start",
        ...
        home: "Home",
        exit: "Exit",
        updateReady: "A new version is ready — tap to update",
        localeSwitch: "中",
