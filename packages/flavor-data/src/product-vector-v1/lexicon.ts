...
import { DIMENSIONS, add, cosine, normalize, zero, type Locale, type Vector } from "./engine";
...
const bank = bundle.question_bank as Record<string, { question: string; options: Record<string, { label: Record<Locale, string>; cues: Record<Locale, string[]> }> }>;

export type MatchedTerm = { term: string; dimension?: string; concept?: string; negated: boolean };
export type OptionScore = { option: string; cosine: number; cueHits: number; score: number; forced: boolean };
...
function isNegated(text: string, index: number, locale: Locale): boolean {
  ...
  return mapper.negation_prefixes[locale].some((prefix) => window.endsWith(prefix) || window.endsWith(prefix.trim()));
}
...
export function utteranceVector(text: string, locale: Locale): { vector: Vector; matched: MatchedTerm[] } {
  let sum = zero();
  ...
  const consider = (term: string, vec: Vector, meta: Omit<MatchedTerm, "term" | "negated">) => {
...
export function mapUtterance(text: string, locale: Locale): UtteranceMapping {
  ...
  const forcing = mapper.forced_by_negation[locale] as unknown as Record<string, [string, string]>;
  for (const [word, [slot, option]] of Object.entries(forcing)) {
    const negatedAt = occurrences(text, word).filter((index) => isNegated(text, index, locale));
    if (negatedAt.length) {
  ...
  for (const slot of Object.keys(bank) as Slot[]) {
    ...
    const projectionMass = Math.sqrt(projected.reduce((acc, x) => acc + x * x, 0));
    const direction = mapper.direction_questions.includes(slot);
    const rows: OptionScore[] = Object.entries(options).map(([option, spec]) => {
      const optionVector = slotVector(slot, option);
      const cos = direction && projectionMass >= mapper.min_projection && optionVector.some((x) => x > 0) ? cosine(projected, optionVector) : 0;
      let cueHits = 0;
      for (const cue of spec.cues[locale] ?? []) {
        for (const index of occurrences(text, cue)) {
          ...
          const encodesNegation = mapper.negation_prefixes[locale].some((p) => cue.startsWith(p.trim()));
      ...
      return { option, cosine: cos, cueHits, score: mapper.alpha * cos + mapper.beta * cueHits + (isForced ? 10 : 0), forced: isForced };
    });
    rows.sort((a, b) => b.score - a.score);
    scores[slot] = rows;
    if (forced[slot]) continue;
    const best = rows[0]!;
    const evidence = best.cueHits > 0 || (direction && projectionMass >= mapper.min_projection && best.cosine > 0);
