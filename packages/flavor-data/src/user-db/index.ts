...
export class UserBeanDatabase extends Dexie {
  ...
  constructor(name = "flavorwords-user-db") {
    ...
    this.version(1).stores({
      user_beans: "++id, name, roast_level, process, variety, created_at",
    });
  }
}
...
export async function addBean(
  db: UserBeanDatabase,
  bean: Omit<UserBean, "id" | "created_at"> & { created_at?: number },
): Promise<number> {
  if (!bean.name.trim()) throw new Error("bean needs a name");
  const conceptIds = [
    ...new Set(bean.concept_ids.filter((id) => id.startsWith("sensory."))),
  ];
  return db.user_beans.add({
    ...bean,
    concept_ids: conceptIds,
    created_at: bean.created_at ?? Date.now(),
  });
}
...
export async function beansAsVectors(
  db: UserBeanDatabase,
): Promise<BeanVector[]> {
  ...
  return beans
    ...
    .map((b) => ({
      id: String(b.id),
      label: b.name,
      vector: projectConcepts(b.concept_ids),
      source: "user" as const,
      conceptIds: b.concept_ids,
    }));
}

/** Parse a label's tasting-note words into canonical concept ids through the concept tag table (zh or en). */
export function conceptIdsFromLabel(
  text: string,
  conceptTags: Record<string, { "zh-CN": string; en: string }>,
): string[] {
  const low = text.toLowerCase();
  const ids: string[] = [];
  for (const [concept, tags] of Object.entries(conceptTags)) {
    if (
      (tags["zh-CN"] && low.includes(tags["zh-CN"].toLowerCase())) ||
      (tags.en && low.includes(tags.en.toLowerCase()))
    )
      ids.push(concept);
  }
  return ids;
