...
def run(owner):
    ...
    fitlog = []
    for C in plan["C"]:
        label = "M2_R1_CONDITIONAL_C" + str(C)
        rows = []
        for fold in range(3):
            ...
            if path.exists():
                ...
                b, rec = tr.fit(
                    ...
                    bank_override=old["question_bank"],
    ...
    if path.exists():
        ...
        b, rec = tr.fit(
            ...
            tag=chosen + ":all-development",
            bank_override=oldfull["question_bank"],
            loss_mode="layered_conditional",
        )
        b["experiment_variant"] = chosen
        save(path, b)
    ...
    save(dst / f"cv/{chosen}_historical.private.json", hist)
    save(dst / "conditional_fit_log.private.json", fitlog)
    binary = read(dst / "cv/M2_R1_FIXED_LAYERED_LOCKED.private.json")
    legacy = read(dst / "cv/M2_R1_FIXED_LEGACY_LOCKED.private.json")
    old = read(dst / "cv/M2_JOINT_REPRODUCED.private.json")
    metrics = read(OUT / "metrics.json")
    metrics["conditional_recovery_repair"] = {
        ...
        "selected_C": C,
        "historical": compact_summary(hist),
        "fit_count": len(fitlog),
        "all_variants_retained": True,
        "original_binary_failure_unchanged": True,
    }
    save(OUT / "metrics.json", metrics)
    save(
        dst / "conditional_selection.private.json",
        {
            "model": chosen,
            "C": C,
            "loss_mode": "layered_conditional",
