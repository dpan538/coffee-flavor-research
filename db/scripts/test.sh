...
if (( migration_count > 53 )); then
  printf 'Running Round 3L acquisition validation contract on database %s.\n' "$TARGET_DATABASE"
  psql \
    -X \
    --set=ON_ERROR_STOP=1 \
    --dbname="$TARGET_DATABASE" <<'SQL'
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Round 3L acquisition validation failed: one or more checks reported violations';
  END IF;
END
