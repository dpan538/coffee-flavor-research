...
def fixed_terms():
    ...
    mapping.update(
        {
            "berries": "attribute.fruity",
            "fruits": "attribute.fruity",
            ...
            "citrus": "broad.citrus",
            "spices": "attribute.spices",
            "green vegetative": "attribute.green_vegetative",
            "raspberries": "sensory.raspberry",
            "blueberries": "sensory.blueberry",
            "blackberries": "sensory.blackberry",
            "strawberries": "sensory.strawberry",
            "cherries": "sensory.cherry",
            "peaches": "sensory.peach",
            "raisins": "sensory.raisin",
...
SPANISH_EXACT = {
    "manzana": "sensory.apple",
    "naranja": "sensory.orange",
    "limón": "sensory.lemon",
    "ciruela": "sensory.plum",
    "durazno": "sensory.peach",
    "uva": "sensory.grape",
    "uvas": "sensory.grape",
    "mango": "sensory.mango",
    "piña": "sensory.pineapple",
    "cerezas": "sensory.cherry",
    "moras": "sensory.blackberry",
    "arándanos": "sensory.blueberry",
    "arandanos": "sensory.blueberry",
    "toronja": "sensory.grapefruit",
    "pasas": "sensory.raisin",
    "guindones": "sensory.prune",
    "miel": "sensory.honey",
    "caramelo": "sensory.caramel",
    "melaza": "sensory.molasses",
    "vainilla": "sensory.vanilla",
    "azúcar morena": "sensory.brown_sugar",
    "almendra": "sensory.almond",
    "almendras": "sensory.almond",
    "avellanas": "sensory.hazelnut",
    "avellana": "sensory.hazelnut",
    "maní": "sensory.peanut",
    "malta": "sensory.malt",
    "canela": "sensory.cinnamon",
    "cedro": "sensory.cedar",
    "madera": "sensory.woody",
    "manzanilla": "sensory.chamomile",
    "te verde": "sensory.green_tea",
    "té verde": "sensory.green_tea",
    "te negro": "sensory.black_tea",
    "té negro": "sensory.black_tea",
    "cocoa": "sensory.cocoa",
    "chocolate": "broad.chocolate",
    "floral": "attribute.floral",
    "frutas": "attribute.fruity",
    "frutos rojos": "attribute.fruity",
...
def parse_terms(raw, language="en"):
    ...
    for phrase in re.split(r"[,;\n.]", str(raw or "")):
        ...
        if language == "es":
            term = re.sub(
                r"^(?:notas?\s+(?:a\s+|de\s+)?|base\s+(?:de\s+)?|fondo\s+(?:a\s+|de\s+)?)",
                "",
                term,
            )
        if not term:
...
def ordinal(raw, meaning):
    ...
    scale = {
        ...
        "numeric_encoding": "rank 0..4; not interval intensity",
...
def zenodo_views(owner=PRIVATE):
    ...
    cells = list(
        openpyxl.load_workbook(source, read_only=True, data_only=True)[
            "All Panelists"
        ].values
    )
    ...
    for rn, row in enumerate(cells[2:], 3):
        ...
        gid = (
            "coffee-group:" + digest((ZENODO_FAMILY + "|" + name).encode())[:24]
            if name
            else "zenodo:UNRESOLVED_ANONYMOUS_LOTS"
        )
        terms, unmatched = set(), []
        for col in [4, 6, 8, 10, 13, 16, 19]:
            t, u = parse_terms(row[col])
            terms.update(t)
            unmatched.extend(u)
        values = {
            key: ordinal(row[col], meaning)
            for col, key, meaning in [
                (
                    11,
                    "native.acidity_intensity",
                    "source acidity intensity; not acidity quality score",
                ),
                (
                    14,
                    "taste.sweetness",
                    "sweetness intensity; separate from sweet-associated aroma",
                ),
                (17, "taste.bitterness", "bitterness intensity; separate from quality"),
            ]
        }
        rec = {
            "record_id": "zenodo:r1:panel-row:" + str(rn),
            "source_family": "zenodo",
            "sample_id": sid,
            "group_id": gid,
            "evaluation_group": gid,
            "evidence_unit_ids": ["zenodo:20840464:All Panelists:row:" + str(rn)],
            "targets": sorted(terms),
            "relevance": {t: 1 for t in terms},
            "attribute_measurements": values,
            "unmapped_source_phrases": unmatched,
            "source_C0": None,
            "source_C1": None,
            "source_native_C1_historical": row[3],
            "role": "CORE_PROFESSIONAL",
            "supervision": "INCOMPLETE_POSITIVE_DESCRIPTIONS_AND_SEPARATE_ORDINAL_INTENSITIES",
            "split": old_groups.get(gid, "QUARANTINED_UNKNOWN_LOT_OVERLAP"),
            "lot_identity_status": (
                "D0_KNOWN_GROUP" if name else "UNKNOWN_POSSIBLE_D0_OVERLAP"
            ),
            "task_masks": {
                "attribute": True,
                "leaf_recovery": bool(terms),
                "core_d0_comparison": name is not None,
                "new_confirmation": False,
            },
            "proxy_status": "DERIVED_RECORD_PROXY",
            "missing_descriptor_status": "NOT_MENTIONED",
        }
    ...
    for sid, records in sorted(grouped.items(), key=lambda x: int(x[0])):
        ...
        for key in records[0]["attribute_measurements"]:
            ...
            values[key] = {
                "panelist_measurements": original,
        ...
        aggregate.append(
            {
                **base,
                "record_id": "zenodo:r1:sample:" + sid,
                "targets": sorted(mentions),
                "relevance": dict(mentions),
                "panelist_mention_sets": [r["targets"] for r in records],
                "attribute_distributions": values,
                "evidence_unit_ids": [
                    e for r in records for e in r["evidence_unit_ids"]
                ],
    ...
    known = [
        r for r in observations if not r["lot_identity_status"].startswith("UNKNOWN")
    ]
    return {
        "raw_rows": len(cells) - 2,
        "panelist_observations": len(observations),
        "source_sample_ids": len(grouped),
        "d0_known_groups": len({r["group_id"] for r in known}),
        "d0_new_attribute_observations": len(known),
        "anonymous_source_sample_ids": len(anonymous),
        "anonymous_panelist_observations": sum(
            r["panelist_observation_count"] for r in anonymous
        ),
        "proven_new_independent_lots": 0,
        "strict_D0_D1_comparison_admission": False,
...
def peru_records(owner=PRIVATE):
    ...
    for ppc in range(8):
        for stage in range(5):
            ...
            for ti, temp in enumerate([40, 50, 60]):
                ...
                condition = f"PPC_{chr(65 + ppc)}:" + (
                    "baseline" if stage == 0 else f"{temp}C:day{int(day)}"
                )
                targets, unmatched = parse_terms(raw, "es")
                evidence = f"peru:FileS1:Descriptors:row{rn+1}:column{ti*3+3}"
                records.append(
                    {
                        "record_id": "peru_2026:" + condition,
                        "group_id": group,
                        "evaluation_group": group,
                        "source_family": "peru_2026",
                        "condition_id": condition,
                        "targets": targets,
                        "relevance": {t: 1 for t in targets},
                        "evidence_unit_ids": [evidence],
                        "raw_source_description": raw,
                        "source_language": "es",
                        "unmapped_source_phrases": unmatched,
                        "panelist_mention_sets": [],
                        "source_C0": "preparation.family.immersion",
                        "source_C1": None,
                        "source_native_C1_historical": (
                            "medium_source_native_unmapped" if ppc >= 3 else None
                        ),
                        "source_context": {
                            "packaging_product_configuration": chr(65 + ppc),
                            ...
                            "panelist_count": 6,
                            "narrative_unit": "published concatenation of graders; not six separable answers",
                        },
                        "quality_score": measurement(
                            score,
                            {"type": "source cupping quality total"},
                        ...
                        "proxy_status": "DERIVED_RECORD_PROXY",
                        "missing_descriptor_status": "NOT_MENTIONED",
                        "recovery_target_available": bool(targets),
                        "task_masks": {
                            "attribute": False,
                            "leaf_recovery": bool(targets),
                            "core_evaluation": False,
                            "new_confirmation": False,
                        },
        ...
        blocks = [
            i for i, row in enumerate(sheet) if row[0] in ["40 °C", "50 °C", "60 °C"]
        ]
        assert len(blocks) == 3
                ...
                for judge in range(6):
                    ...
                    for row in sheet[start + 3 : start + 14]:
                        ...
                        values[row[1]] = (
                            measurement(
                                float(raw),
                                {"type": "source cupping quality"},
                                "grader mean over three packages; not intensity",
                            )
                    ...
                    quality.append(
                        {
                            ...
                            "storage_temperature_c": (
                                None if stage == 0 else [40, 50, 60][ti]
                            ),
                            "stage": stage,
                            "judge_local_index": judge + 1,
                            "values": values,
    ...
    return records, {
        "raw_descriptor_cells": 120,
        "duplicate_baseline_descriptor_cells": 16,
        "unique_condition_narratives": 104,
        "grader_mean_quality_observations": 624,
        "independent_coffee_lot_groups": 1,
        "independent_collection_studies": 1,
        "admitted_recovery_records": len(records),
        "nonempty_positive_recovery_records": sum(
            r["recovery_target_available"] for r in records
        ),
        "fine_descriptor_records": sum(
            any(t.startswith("sensory.") for t in r["targets"]) for r in records
        ),
        "complete_intensity_records": 0,
        "source_context_paired_records": 104,
        "production_C0_C1_paired_records": 0,
        "confirmation_groups": int(split(group) == "CONFIRMATION"),
        "group_id": group,
        "split": split(group),
        "role": "AUX_COFFEE_WEAK_LABEL",
        ...
        "url": "https://doi.org/10.3390/foods15152756",
        "version": "2026-08-05 Version of Record",
        ...
        "archive_sha256": digest((root / "peru-attachment.zip").read_bytes()),
        "xlsx_sha256": digest(content),
...
def cotter_records(owner=PRIVATE):
    ...
    blob_hash = hashlib.sha1(
        b"blob " + str(artifact.stat().st_size).encode() + b"\0" + artifact.read_bytes()
    ).hexdigest()
    assert any(
        x["path"] == "models/no_aggregation/output_reg/preprocessing/_data_none.csv"
        and x["sha"] == blob_hash
        for x in tree["tree"]
    )
    ...
    for i, row in enumerate(rows):
        values = {
            concept: measurement(
                int(row[name]),
                {"type": "binary CATA", "min": 0, "max": 1},
                "0=not detected on shown CATA ballot; 1=detected; consumer report",
            )
            for name, concept in CATA_CONCEPTS.items()
        }
        assert all(m["value"] in (0, 1) for m in values.values())
        result.append(
            {
                "record_id": "cotter:author-derived-row:" + str(i),
                "group_id": group,
                "evaluation_group": group,
                ...
                "relevance": {
                    c: m["value"] for c, m in values.items() if m["value"] == 1
                },
                ...
                "panelist_id": None,
                "evidence_unit_ids": [
                    "cotter:github:" + tree["sha"] + ":unaggregated:" + str(i + 2)
                ],
    ...
    return result, {
        "original_api_files_acquired": False,
        "original_download_failure": "HTTP401 API / HTTP403 public file_stream",
        ...
        "raw_original_rows_acquired": 0,
        "author_derived_observation_rows_acquired": len(result),
        "available_complete_CATA_columns": list(CATA_CONCEPTS),
        "sensory_cell_count": len(result) * len(CATA_CONCEPTS),
        "independent_coffee_lot_groups": 1,
        "independent_collection_studies": 1,
        "source_reported_conditions": 27,
        "condition_ids_recoverable": 0,
        "fine_grained_core_training_records": 0,
        "broad_attribute_auxiliary_records": len(result),
        "production_C0_C1_paired_records": 0,
        "confirmation_groups": int(split(group) == "CONFIRMATION"),
        "group_id": group,
        "split": split(group),
        ...
        "url": "https://doi.org/10.25338/B8993H",
        "version": "Dryad 2023-01-16; author derivative Git tree " + tree["sha"],
        "license": "CC0 1.0 original data; Apache-2.0 author derivative repository",
        "license_urls": [
            "https://spdx.org/licenses/CC0-1.0.html",
            "https://github.com/mhgun/sensory_prediction/blob/"
            + tree["sha"]
            + "/LICENSE",
        ],
        ...
        "artifact_url": "https://github.com/mhgun/sensory_prediction/blob/"
        + tree["sha"]
        + "/models/no_aggregation/output_reg/preprocessing/_data_none.csv",
...
def cotter_full_records(owner=PRIVATE):
    ...
    assert (
        digest(readme.read_bytes())
        == "f6d8f508bad2824a27be8785c841e8df4c75751b58726820f9e3dd226fe3fb5e"
    )
    assert "CC0 1.0" in readme.read_text(encoding="cp1252")
    for name, raw in [
        ("cotter-original-cache.csv", source.read_bytes()),
        ("cotter-original-cache-README.txt", readme.read_bytes()),
    ]:
    ...
    columns = {
        **CATA_CONCEPTS,
        "Citrus": "broad.citrus",
        "Green.veg": "compound.green_vegetative",
        "Paper.wood": "compound.paper_wood",
        "Cereal": "native.cereal",
        "Thick.viscous": "mouthfeel.thick_viscous",
    }
    rows = list(csv.DictReader(source.open()))
    assert len(rows) == 3186 and len(columns) == 17
    assert (
        len({r["Judge"] for r in rows}) == 118 and len({r["Brew"] for r in rows}) == 27
    )
    ...
    for i, row in enumerate(rows):
        values = {
            concept: measurement(
                int(row[name]),
                {"type": "binary CATA", "min": 0, "max": 1},
                "0=not detected on explicitly shown CATA ballot; 1=detected; consumer observation",
            )
            for name, concept in columns.items()
        }
        assert all(m["value"] in (0, 1) for m in values.values())
        records.append(
            {
                "record_id": "cotter:original-row:" + str(i + 2),
                "source_family": "cotter_2023",
                "group_id": group,
                "evaluation_group": group,
                "split": split(group),
                "role": "AUX_COFFEE_WEAK_LABEL",
                "panelist_id": "cotter:judge:" + row["Judge"],
                "condition_id": "cotter:brew:" + row["Brew"],
                "evidence_unit_ids": [
                    "cotter:original:" + row["Judge"] + ":" + row["Brew"]
                ],
                "attribute_measurements": values,
                "targets": sorted(c for c, m in values.items() if m["value"] == 1),
                ...
                "task_masks": {
                    "attribute": True,
                    "attribute_positive_mentions": True,
                    "attribute_intensity": False,
                    "attribute_binary_detection": True,
                    "leaf_recovery": False,
                    "core_evaluation": False,
                    "context_row_link": True,
                    "new_confirmation": False,
                },
                "proxy_status": "DERIVED_RECORD_PROXY",
            }
    ...
    return records, {
        "status": "REUSED_GOVERNED_ORIGINAL_CACHE_HASH_VERIFIED",
        "raw_original_rows_acquired": 3186,
        "panelist_count": 118,
        "condition_count": 27,
        "measured_CATA_columns": list(columns),
        "sensory_cell_count": 3186 * 17,
        "independent_coffee_lot_groups": 1,
        "incremental_coffee_groups_over_author_derivative": 0,
        "new_to_D0_collection_source": True,
        "newly_downloaded_collection_source_this_block": False,
        "author": "Andrew Cotter; William D. Ristenpart; Jean-Xavier Guinard",
        "url": "https://doi.org/10.25338/B8993H",
        "version": "Dryad v4 2023-01-16; data collected April–June 2019",
        "license": "CC0 1.0",
        "license_url": "https://creativecommons.org/publicdomain/zero/1.0/",
        "conditions_satisfied": True,
        "cache_provenance": "db/data/round3b/raw/cotter_2020_black_coffee; original hash equals live Dryad v4 file metadata",
        "artifact_sha256": digest(source.read_bytes()),
        "readme_sha256": digest(readme.read_bytes()),
        "private_artifact": str(
            owner / "cotter_full_attribute_observations.private.json"
        ),
...
CONDELLI_CONCEPTS = {
    ...
    "Caramel": "sensory.caramel",
    "Licorice": "native.licorice",
    "Chocolate": "broad.chocolate",
    "Nutty": "broad.nutty",
    "Roasted": "attribute.roasted",
    "Burnt": "native.burnt",
    "Earthy": "sensory.earthy",
    "Grassy": "native.grassy",
    "Citrus": "broad.citrus",
    "Bitter": "taste.bitterness",
    "Sweet": "taste.sweetness",
    "Acidic": "taste.sourness",
    "Astringent": "mouthfeel.astringent",
}
...
def condelli_records(owner=PRIVATE):
    ...
    registration = {
        "version": "m2-r1-condelli-split.v1",
        "seed": SPLIT_SEED,
        ...
        "groups": {
            sid: {"group_id": group, "split": split(group)}
    ...
    assert any(
        "by/4.0" in x
        for x in xml.xpath('//permissions//@*[contains(.,"creativecommons")]')
    )
    archive = zipfile.ZipFile(root / "mono-epmc-supp.zip")
    content = archive.read("JFDS-87-4688-s001.docx")
    doc = etree.fromstring(
        zipfile.ZipFile(io.BytesIO(content)).read("word/document.xml")
    )
    ...
    def table_rows(index):
        return [
            ...
            for row in doc.xpath("//w:tbl", namespaces=ns)[index].xpath(
                "./w:tr", namespaces=ns
            )
        ]
    ...
    native = {
        re.sub(r"\s*\*+$", "", row[0]): [int(v) for v in row[1:]] for row in cells[2:]
    }
    ...
    scale = {
        "type": "source-reported rounded CATA citation percentage",
        "min": 0,
        "max": 100,
        "reporting_resolution": 1,
        "respondent_denominator": None,
        "zero_means_absence": False,
        "not_intensity": True,
    ...
    vocabulary = set(BASE_CANDIDATES) | {
        c
        for c in CONDELLI_CONCEPTS.values()
        if c.startswith(("broad.", "attribute.", "taste.", "mouthfeel."))
    }
    for col, sid in enumerate(CONDELLI_IDS):
        attrs = {
            ...
            for label, concept in CONDELLI_CONCEPTS.items()
        }
        positive = sorted(
            c for c, value in attrs.items() if value["value"] > 0 and c in vocabulary
        )
        base = {
            "record_id": "condelli:tableS8:" + sid,
            "source_family": "condelli_2022",
            "sample_id": sid,
            "group_id": groups[sid],
            "evaluation_group": groups[sid],
            "split": split(groups[sid]),
            "role": "AUX_COFFEE_WEAK_LABEL",
            "source_C0": "preparation.family.espresso_pressure",
            "source_C1": None,
            ...
            "source_identity_metadata": {
                "continent": identity[sid][0],
                "country": identity[sid][1],
                "species_or_product": identity[sid][2],
            },
            "lot_identity_status": "SOURCE_DEFINED_COFFEE_PRODUCT; RAW_LOT_NOT_REPORTED; BLEND_CONSTITUENTS_UNKNOWN",
            "targets": positive,
            "relevance": {c: attrs[c]["value"] for c in positive},
            "attribute_measurements": attrs,
            "source_native_all_citation_percentages": {
                k: v[col] for k, v in native.items()
            },
            ...
            "task_masks": {
                ...
                "attribute_positive_mentions": True,
                "attribute_intensity": False,
                "attribute_binary_detection": False,
                "native_citation_profile": True,
                "leaf_recovery": bool(positive),
    ...
    return recovery, {
        "author": "Nicola Condelli; Nazarena Cela; Maria Di Cairano; Teresa Scarpa; Luigi Milella; Roberta Ascrizzi; Guido Flamini; Fernanda Galgano",
        "url": "https://doi.org/10.1111/1750-3841.16323",
        "version": "2022 version of record",
        "license": "CC BY 4.0",
        "license_url": "https://creativecommons.org/licenses/by/4.0/",
        "conditions_satisfied": True,
        "attribution_required": True,
        "artifact_url": "https://www.ebi.ac.uk/europepmc/webservices/rest/PMC9826037/supplementaryFiles",
        "artifact_sha256": digest((root / "mono-epmc-supp.zip").read_bytes()),
        "docx_sha256": digest(content),
        "source_reported_panelists": 77,
        "raw_panelist_records_acquired": 0,
        "source_defined_coffee_product_groups": 20,
        "traceable_raw_lots": None,
        "sample_id_groups": registration["groups"],
        "independent_collection_studies": 1,
        "admitted_positive_recovery_rows": len(recovery),
        "source_native_cells": len(native) * 20,
        "retained_sensory_citation_cells": len(CONDELLI_CONCEPTS) * 20,
        "intensity_cells": 0,
        "true_zero_sensory_cells": 0,
        "production_C0_C1_paired_records": 0,
        "role": "AUX_COFFEE_WEAK_LABEL",
...
def rocchetti_records(owner=PRIVATE):
    ...
    assert any(
        ...
        for v in xml.xpath("//permissions//@*")
    )
    ...
    registration = json.loads(
        (owner / "rocchetti_split_preregistered.private.json").read_text()
    )
    expected = {
        "rocchetti_2020:coffee_product:"
        + r[0]: split("rocchetti_2020:coffee_product:" + r[0])
        for r in rows[1:]
    }
    ...
    for row in rows[1:]:
        ...
        for label, key in ROCCHETTI_FIELDS.items():
            ...
            body = label == "BODY"
            scale = {
                "type": "source-native 10-position descriptive rating; published panel aggregate",
                "min": 0,
                "max": 9,
                "anchors": ["ZERO", "MAX"],
                ...
                "source_aggregation_method": "NOT_REPORTED; do not assume arithmetic mean",
                "not_quality_score": True,
                "body_zero_reference": (
                    "source filter-coffee viscosity baseline; not absence of body"
                    if body
                    else None
                ),
            }
            attrs[key] = measurement(
                value,
                scale,
                ...
                "OBSERVED" if body or value > 0 else "TRUE_ZERO",
                raw[label],
            )
            attrs[key]["source_label"] = label
        observations.append(
            {
                "record_id": "rocchetti:tableS1:" + row[0],
                "source_family": "rocchetti_2020",
                "sample_id": row[0],
                "group_id": gid,
                "evaluation_group": gid,
                "split": expected[gid],
                ...
                "source_C0": None,
                "source_C1": None,
                "source_native_category_code": row[2],
                "source_submission_country": row[1],
                ...
                "lot_identity_status": "SOURCE_DEFINED_COFFEE_PRODUCT; RAW_LOT_NOT_REPORTED; BLEND_CONSTITUENTS_UNKNOWN",
                "attribute_measurements": attrs,
                "attribute_masks": {k: True for k in attrs},
                "targets": [],
                "relevance": {},
                "task_masks": {
                    "attribute": True,
                    "attribute_intensity": True,
                    "attribute_positive_mentions": False,
                    "attribute_binary_detection": False,
                    "leaf_recovery": False,
                    "source_native_attribute_profile": True,
                    "new_confirmation": expected[gid] == "CONFIRMATION",
                    "production_context_complete": False,
                },
                "evidence_unit_ids": [
                    "rocchetti:PMC7736008:Supplement:TableS1:sample:" + row[0]
                ],
                "proxy_status": "DERIVED_RECORD_PROXY",
                ...
                "source_panel": {
                    "total_experts": 30,
                    "experience": ">1200 hours evaluating food/beverages including coffee",
                    "commissions": 5,
                    "sample_specific_panelist_count": None,
                    "native_values_are": "published panel aggregate",
                },
                "excluded_fields": [k for k in header[3:] if k not in ROCCHETTI_FIELDS],
                "excluded_input_information": [
                    "award/quality stratum",
                    "country",
                    "sample/category IDs",
    ...
    return observations, {
        "author": "Gabriele Rocchetti; Gian Paolo Braceschi; Luigi Odello; Terenzio Bertuzzi; Marco Trevisan; Luigi Lucini",
        "url": "https://doi.org/10.1007/s11306-020-01751-6",
        "version": "2020 version of record; original collection November 2018",
        "license": "CC BY 4.0",
        "license_url": "https://creativecommons.org/licenses/by/4.0/",
        "attribution_required": True,
        "conditions_satisfied": True,
        ...
        "artifact_sha256": digest((root / "metabolomics47.zip").read_bytes()),
        "docx_sha256": digest(content),
        "source_defined_coffee_product_groups": 47,
        "traceable_raw_lots": None,
        "independent_collection_studies": 1,
        "source_reported_panelists": 30,
        "raw_panelist_records_acquired": 0,
        ...
        "admitted_fine_recovery_rows": 0,
        "production_C0_C1_paired_records": 0,
        "role": "CORE_PROFESSIONAL",
        "fresh_brew_eligibility": "NOT_FULLY_DOCUMENTED; product-specific preparation and service timing absent",
        "source_protocol": "Trialtest descriptive/qualitative scores separate; original professionally prepared filter/moka/single-dose/espresso products; product-specific brew family not established by letter codes",
...
def acquisition_audit(owner, close=False):
    ...
    block = {
        ...
        "status": "IN_PROGRESS",
        "authorized_duration_hours": [3, 4],
        "targeted_data_routes_limit": 12,
        "targeted_data_routes_used": 12,
    ...
    previous_manifest = PUBLIC / "sample_expansion_manifest.json"
    previous_block = (
        json.loads(previous_manifest.read_text()).get("source_work_block", {})
        if previous_manifest.exists()
        else {}
    )
    if previous_block.get("ended_utc"):
        block = previous_block
    elif close:
        block.update(
            {
                "ended_utc": now.isoformat(),
                "status": "CLOSED_REGISTERED_ROUTES_EXHAUSTED",
                "duration_budget_used_in_full": False,
                "closure_reason": "Twelve registered data routes were actually checked; final narrow repository checks found no further accessible eligible joint matrix. Three new collections and one governed cache were parsed and handed to training. Closed early without idle waiting or adding unregistered broad searches.",
            }
        )
    specs = [
        (
            "10.3390/foods15152756",
            "ADMITTED_STORAGE_AUXILIARY",
            ["peru-full.xml", "peru-attachment.zip"],
        ...
        (
            "10.25338/B8993H",
            "REUSED_GOVERNED_ORIGINAL_CACHE",
            [
                "cotter-api.json",
                "cotter-original-cache.csv",
                "cotter-original-cache-README.txt",
            ],
        ...
        (
            "10.1111/1750-3841.16323",
            "NEW_ACQUISITION_ADMITTED_CONSUMER_AUXILIARY",
            ["liking-mono.xml", "mono-epmc-supp.zip"],
        ...
        (
            ...
            "NO_ELIGIBLE_SENSORY_MATRIX_ACQUIRED",
            ["nir.xml", "mendeley-nir-files.json"],
        ...
        (
            "10.3389/fsufs.2025.1497350",
            "DATA_AVAILABLE_ON_REQUEST_ONLY",
            ["frontiers-ccc.xml"],
        ...
        (
            ...
            "SUPPLEMENT_ACQUIRED_NO_SENSORY_ROWS",
            [
                "carvalho.xml",
                "carvalho-supp.zip",
                "carvalho-dryad-search.json",
                "carvalho-datacite-exact.json",
            ],
            "67 professional RATA coffees described; public Dryad share returns 404, original supplementary DOCX contains grader demographics and water composition only. Exact article-DOI DataCite search returned no dataset; no sensory matrix and no admitted rows. Native article license is CC BY NC ND; a separate public data license was unavailable.",
        ),
        (
            "10.1111/joss.12886",
            "FULL_TEXT_READ_SUPPLEMENT_NOT_RETRIEVABLE",
            ["williams-crossref.json", "williams-institutional.html"],
            "71 professional coffees and 179 follow-on validation samples described. Public original CC BY NC full text read; XLSX HTTP403; browser download emitted an event but no accessible artifact. Institutional SCU inventory lists article PDF and wheel PPTX only, no XLSX. Mixed original-panel and third-party text would need separation; zero rows counted.",
        ),
        (
            "10.1007/s11306-020-01751-6",
            "NEW_ACQUISITION_ADMITTED_PROFESSIONAL_ATTRIBUTES",
            ["metabolomics47.xml", "metabolomics47.zip"],
        ...
        (
            ...
            "SUPPLEMENT_ACQUIRED_RAW_REPOSITORY_UNAVAILABLE",
            ["liang-supp.zip", "liang-dryad-api.json", "liang-dryad-landing.html"],
        ...
        (
            "10.1038/s41598-020-73341-4",
            "SUPPLEMENT_ACQUIRED_MARGINAL_MEANS_ONLY",
            ["batali2020.xml", "batali2020-supp.zip"],
        ...
        (
            ...
            "SUPPLEMENT_ACQUIRED_NO_SENSORY_ROWS",
            ["batali.xml", "batali2022-supp.zip"],
    ...
    for doi, status, files, use in specs:
        artifacts = {
            f: {
                "sha256": digest((owner / "sources" / f).read_bytes()),
                "bytes": (owner / "sources" / f).stat().st_size,
            }
            for f in files
            if (owner / "sources" / f).exists()
        }
        routes.append(
            {
                "doi": doi,
                ...
                "result": status,
                "actual_cached_artifacts": artifacts,
                "model_use_or_obstacle": use,
            }
        )
    return block, routes
...
def validate_normalized(rows):
    assert len({r["record_id"] for r in rows}) == len(rows), "Duplicate record IDs"
    group_splits = {}
    for r in rows:
        gid = r["group_id"]
        if gid in group_splits:
            assert group_splits[gid] == r["split"], "Coffee group crosses splits"
        group_splits[gid] = r["split"]
        assert (
            r["source_C1"] is None
        ), "No newly admitted source supplies calibrated seven-level C1"
        for v in r.get("attribute_measurements", {}).values():
            known = v["status"] in {"OBSERVED", "TRUE_ZERO"}
            assert (v["value"] is not None) == known
            if v["status"] == "TRUE_ZERO":
                assert v["value"] == 0 and v["scale"].get(
                    "zero_means_absence", v["scale"]["type"] == "binary CATA"
                )
        if r["source_family"] == "rocchetti_2020":
            assert not r["targets"] and not r["task_masks"]["leaf_recovery"]
            assert set(r["attribute_measurements"]) == set(ROCCHETTI_FIELDS.values())
            body = r["attribute_measurements"]["mouthfeel.body"]
            assert (
                body["status"] == "OBSERVED" and not body["scale"]["zero_means_absence"]
            )
        if r["source_family"] == "condelli_2022":
            assert all(r["relevance"][target] > 0 for target in r["targets"])
            assert (
                not r["task_masks"]["attribute_intensity"]
                and not r["task_masks"]["attribute_binary_detection"]
            )


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--owner", type=Path, default=PRIVATE)
    ap.add_argument("--close-source-block", action="store_true")
    args = ap.parse_args()
    owner = args.owner
    owner.mkdir(parents=True, exist_ok=True)
    for filename, expected_sha in SOURCE_DIGESTS.items():
        assert digest((owner / "sources" / filename).read_bytes()) == expected_sha, (
            "Frozen source changed: " + filename
        )
    # Register deterministic grouping before adapters, any feature fitting, or evaluation.
    policy = {
        "version": "m2-r1-source-split.v1",
        "seed": SPLIT_SEED,
        "rule": "SHA256(seed|group_id) first 8 hex modulo 5 == 0 => CONFIRMATION; otherwise DEVELOPMENT",
        "fixed_known_groups": {
            g: split(g)
            for g in [
                "peru_2026:single_blend_lot",
                "cotter_2023:honduras_single_coffee",
            ]
        },
        "created_before_model_results": True,
    ...
    if existing.exists():
        assert json.loads(existing.read_text()) == policy
    else:
        save(existing, policy)
    zen = zenodo_views(owner)
    peru, pm = peru_records(owner)
    cotter, cm = cotter_records(owner)
    frozen_recovery = owner / "d1_recovery_records.private.json"
    if frozen_recovery.exists():
        assert (
            json.loads(frozen_recovery.read_text()) == peru
        ), "Frozen D1 v1 changed; create a distinct version instead"
    else:
    ...
    for admitted in [peru, cotter_full, condelli, rocchetti]:
        validate_normalized(admitted)
    expanded_peru = [
        {
            **r,
            "task_masks": {
    ...
    manifest = {
        "experiment_id": "backend-sequential-model-v2/revisions/r1",
        "purpose": "PERSONAL_NONCOMMERCIAL_COFFEE_RESEARCH",
        "status": "ACQUIRED_AND_PARSED",
        "manifest_version": 3,
        "d1_v1_frozen_path": str(owner / "d1_recovery_records.private.json"),
        "d1_v2_expanded_path": str(owner / "d1_recovery_expanded.private.json"),
        "split_policy": policy,
        "sources": {
            "zenodo_recovered_views": zen,
            "peru_2026": pm,
            "cotter_2023_author_derived_frozen": cm,
            "cotter_2023_original": fm,
            "condelli_2022": dm,
            "rocchetti_2020": rm,
        },
        "actual_increment": {
            "new_independent_coffee_lot_or_product_groups": 69,
            "new_study_coffee_lot_groups": 2,
            "new_source_defined_coffee_product_groups": 67,
            "new_to_D0_independent_collection_studies": 4,
            "newly_acquired_coffee_product_or_study_groups_this_block": 68,
            "reused_groups_new_to_D0": 1,
            "newly_acquired_collection_studies_this_block": 3,
            "reused_existing_collection_studies": 1,
            "new_core_professional_product_groups": 47,
            "fresh_brew_eligibility": "NOT_FULLY_DOCUMENTED",
            "new_complete_native_intensity_records": 47,
            "new_complete_native_intensity_cells": 564,
            "new_known_condition_observations": 104,
            "admitted_recovery_records": len(peru) + len(condelli),
            "nonempty_positive_recovery_records": sum(
                bool(r["targets"]) for r in peru + condelli
            ),
            "fine_descriptor_records": sum(
                any(t.startswith("sensory.") for t in r["targets"])
                for r in peru + condelli
            ),
            "new_complete_CATA_auxiliary_records": len(cotter_full),
            "new_confirmation_groups": dm["confirmation_groups"]
            + rm["confirmation_groups"],
            "new_production_C0_C1_paired_records": 0,
            "quarantined_anonymous_source_sample_ids": zen[
                "anonymous_source_sample_ids"
            ],
        },
        "target_gap": {
            "independent_group_target": [50, 100],
            "broad_product_group_shortfall_to_50": 0,
            "broad_source_defined_coffee_or_product_groups": 69,
            "broad_group_target_status": "MET_USING_SOURCE_DEFINED_PRODUCT_GROUPS",
            "explicit_single_study_coffee_or_lot_groups": 2,
            "other_product_groups_with_unverified_raw_lot_independence": 67,
            "strict_traceable_raw_lot_count": None,
            "strict_raw_lot_target_status": "NOT_ESTABLISHED_AS_MET; traceable lot identity and independent ingredients unavailable for product groups",
            "strict_raw_lot_shortfall_to_50": None,
        ...
        "not_evaluated": [
            ...
            "NEW_CONFIRMATION_RESULTS_AT_ACQUISITION_TIME",
        ],
        "private_artifacts": {
            p.name: {"path": str(p), "sha256": digest(p.read_bytes())}
            for p in owner.glob("*private.json")
            if any(
                k in p.name
                for k in [
                    "d1_",
        ...
        "supervision_mask_contract": {
            "legacy_attribute_field": "Legacy field refers to complete numeric measurement supervision, never a blanket prohibition on positive broad mentions.",
            "Peru": {
                "attribute_positive_mentions": True,
                "attribute_intensity": False,
                "attribute_binary_detection": False,
            ...
            "Rocchetti": {
                "attribute_positive_mentions": False,
                "attribute_intensity": True,
                "intensity_type": "SOURCE_NATIVE_10_POSITION_PANEL_AGGREGATE",
                "attribute_binary_detection": False,
                "leaf_recovery": False,
            },
            "Cotter": {
            ...
            "Zenodo": {
                "attribute_positive_mentions": True,
                "attribute_intensity": True,
                "intensity_type": "SOURCE_NATIVE_ORDINAL_ONLY",
                "attribute_binary_detection": False,
            },
            "frozen_v1": "Earlier 104-row Peru artifact remains byte-identical; this contract clarifies its legacy attribute=false meaning.",
        },
        "measurement_warning": "Cupping quality, JAR adequacy, aroma association and basic-taste intensity have separate fields and masks.",
    }
    manifest["source_work_block"], manifest["acquisition_routes"] = acquisition_audit(
        owner, close=args.close_source_block
    )
    manifest["confirmation_cohorts"] = {
        "consumer_condelli": {
            "groups": 3,
            "source_family": "condelli_2022",
            "meaning": "consumer positive-only recovery; already inspected separately",
        },
        "professional_rocchetti": {
            "groups": 9,
            "source_family": "rocchetti_2020",
            "meaning": "new independent product-group confirmation for native attribute heads; not pooled with consumer confirmation",
        },
    }
    for family, rows in [("peru_2026", peru), ("condelli_2022", condelli)]:
        target_source = manifest["sources"][family]
        target_source.pop("fine_grained_recovery_records", None)
        target_source.update(
            {
                "admitted_recovery_records": len(rows),
                "nonempty_positive_recovery_records": sum(
                    bool(r["targets"]) for r in rows
                ),
                "fine_descriptor_records": sum(
                    any(t.startswith("sensory.") for t in r["targets"]) for r in rows
                ),
                "direct_attribute_prefix_records": sum(
                    any(t.startswith("attribute.") for t in r["targets"]) for r in rows
                ),
                "direct_broad_prefix_records": sum(
                    any(t.startswith("broad.") for t in r["targets"]) for r in rows
                ),
                "empty_parsed_target_records": sum(not r["targets"] for r in rows),
                "coverage_note": "Rows with no accepted parsed targets remain in the coverage denominator. Prefix counts overlap and are not added; broad classes inferred later from leaves are not counted as direct broad mentions.",
            }
        )
    manifest["paper_reading_scope"] = {
        "requested_original_papers": 7,
        "requested_full_texts_read": 6,
        "requested_abstract_only": ["Frost2020"],
        "additional_original_full_texts_in_compact_table": ["Batali2020"],
        "compact_table_full_original_texts_read": 7,
        "limitation": "Six of the seven requested original full texts read; do not count supplementary Batali2020 as completing Frost.",
    }
    manifest["verification"] = {
        "original_source_sha256_assertions": len(SOURCE_DIGESTS),
        "frozen_D1_v1_preserved": True,
        "record_ids_unique": True,
        "coffee_groups_do_not_cross_splits": True,
        "missing_is_not_zero": True,
        "compound_categories_remain_whole": True,
        "Rocchetti_targets_empty_attribute_only": True,
        "Condelli_rounded_zeros_not_negative_labels": True,
    }
