    ...
    return {
        ...
        **{k: mean(rows, k) for k in keys},
        "per_source": {
            src: {
                "groups": len(
                    {r["group_id"] for r in rows if r["source_family"] == src}
                ),
                **{
                    k: mean([r for r in rows if r["source_family"] == src], k)
                    for k in keys
                },
            }
...
def comparison(a, b, key="ndcg5"):
    ...
    if value["status"] == "IMPROVEMENT_SUPPORTED":
        value["status"] = "PROXY_IMPROVEMENT_SUPPORTED"
    value["inference"] = (
        "Development selection or historical record proxy; not independent product efficacy."
    )
    return value
...
def freeze(owner):
    ...
    paths = [
        ...
        ROOT / "db/data/backend-sequential-model-v2/dataset_manifest.json",
    ]
    paths += list((owner / "models").glob("*.json")) + list(
        (owner / "cv").glob("*.model.json")
    )
    ...
    cfg = {
        ...
        "D0": {
            ...
            "historical_groups": len(
                {
                    r["group_id"]
                    for r in records
                    if r["split"] == "HISTORICAL_REGRESSION"
                }
            ),
            "split_sha256": old_s.digest(folds),
            "immutable_file_sha256": frozen,
        },
        "stages": [
            {
                "id": "repair_checkpoint",
                "data": "D0",
                "bank": "old train-fold bank locked",
                "targets": "old visible_episode fixed hidden recovery",
                "C": "old selected C",
            ...
            {
                "id": "complementary_Q01",
                "data": "D0",
                "models": ["M2_R1_FIXED_LAYERED_LOCKED", "M2_R1_FIXED"],
            ...
            },
            {
                ...
                "data": "D0",
                "slot": "Q3",
                "optional_control_slot": "Q4 only if preregistered before run",
            ...
            {
                "id": "expansion",
                "models": ["M2_R1_FIXED_D0", "M2_R1_FIXED_D0_D1"],
                "locked": [
        ...
        "backend_default": "B2 (unchanged pending evidence)",
    }
...
def scope_row(row, vocabulary=None, kind="full"):
    x = copy.deepcopy(row)
    ep = x["episode"]
    targets = dict(ep["relevance"])
    if vocabulary is not None:
        targets = {c: v for c, v in targets.items() if c in vocabulary}
    if kind == "fine":
        targets = {c: v for c, v in targets.items() if c.startswith("sensory.")}
    if kind == "broad":
        targets = {c: v for c, v in targets.items() if not c.startswith("sensory.")}
    ranking = x["recovery_ranking"]
    if vocabulary is not None:
        ranking = [c for c in ranking if c in vocabulary]
    if kind == "fine":
        ranking = [c for c in ranking if c.startswith("sensory.")]
    if kind == "broad":
        ranking = [c for c in ranking if not c.startswith("sensory.")]
    x.update(
        candidate_target_coverage=(
            len(set(x["ranking"]) & targets.keys()) / len(targets) if targets else None
        ),
        ndcg5=old_t.ndcg(ranking, targets) if targets else None,
        recall5=(
            len(set(ranking[:5]) & targets.keys()) / len(targets) if targets else None
        ),
        recall8=(
            len(set(ranking[:8]) & targets.keys()) / len(targets) if targets else None
        ),
    )
    return x
...
def enrich(row, record, backend, bundle):
    ...
    if "episode" not in row:
        row["episode"] = old_t.visible_episode(record)
    if "state" in row:
        ...
        row["raw_direct_retention8"] = (
            len({r["candidate_id"] for r in raw[:8]} & direct) / len(direct)
            if direct
            else None
        )
...
def reproduce(owner):
    ...
    for fold in range(3):
        ...
        held = [
            r
            for r in records
            if r["group_id"] in folds and folds[r["group_id"]] == fold
        ]
        for r in held:
            row = enrich(old_t.evaluate_record(r, b), r, old_s, b)
            expected = frozen[r["record_id"]]
            if (
                row["ranking"] != expected["ranking"]
                or row["ndcg5"] != expected["ndcg5"]
            ):
        ...
        print(
            json.dumps({"phase": "old_replay", "fold": fold, "records": len(held)}),
            flush=True,
        )
    if failures:
        raise ValueError("OLD_REPRODUCTION_MISMATCH:" + str(len(failures)))
    ...
    metrics["D0_old_reproduction"] = {
        "identical_ranking_and_utility": True,
        "M2_JOINT": compact_summary(rows),
        "B2": compact_summary(baselines),
        "basis": "Retained fold bundles; original grouped data, catalog and target partition.",
    }
...
def repair(owner):
    ...
    folds = read(dst / "D0_folds.private.json")
    manifest_hash = cfg.get("initial_plan_hash", old_s.digest(cfg))
    oldC = read(ROOT / "db/data/backend-sequential-model-v2/metrics.json")[
        "sequential"
    ]["selected_C"]
    recipes = [
        ("M2_R1_FIXED_LEGACY_LOCKED", "legacy", False, oldC),
        ("M2_R1_FIXED_LAYERED_LOCKED", "layered", False, oldC),
        ("M2_R1_FIXED", "layered", True, oldC),
    ]
    recipes += [
        ("M2_R1_FIXED_C" + str(c), "layered", True, c)
        for c in cfg["bounded_search"]["C"]
        if c != oldC
    ]
    rows_by = {}
    fitlog = []
    for label, loss, new_pair, C in recipes:
        ...
        for fold in range(3):
            ...
            if path.exists():
                b = read(path)
                if b["data_manifest_hash"] != manifest_hash:
                    raise ValueError("RETAINED_R1_MANIFEST_MISMATCH")
                rec = b["fit_receipt"]
            else:
                begin = time.perf_counter()
                bank = copy.deepcopy(old["question_bank"])
                if new_pair:
                    preliminary = tr.make_bundle(
                        train,
                        "M2_R1_FIXED",
                        manifest_hash,
                    ...
                    for key in ["initial_0", "initial_1", "initial_pair_selection"]:
                        bank[key] = preliminary["question_bank"][key]
                b, rec = tr.fit(
                    train,
                    manifest_hash,
                    C=C,
        ...
        if path.exists():
            ...
            if new_pair:
                preliminary = tr.make_bundle(
                    ...
                    "M2_R1_FIXED",
                    manifest_hash,
                    vocabulary=oldfull["candidate_vocabulary"],
                    tag="bank-final",
                )
                for key in ["initial_0", "initial_1", "initial_pair_selection"]:
                    bank[key] = preliminary["question_bank"][key]
            ...
            json.dumps(
                {
                    "phase": "historical",
                    "model": label,
                    "NDCG5": mean(hist[label], "ndcg5"),
                }
            ),
            flush=True,
        )
    save(dst / "cv/historical_r1.private.json", hist)
    save(dst / "fit_log.private.json", fitlog)
    oldrows = read(dst / "cv/M2_JOINT_REPRODUCED.private.json")
    metrics = read(OUT / "metrics.json")
    comparisons = {
        "repair_legacy_minus_old": comparison(rows_by[recipes[0][0]], oldrows),
        "layered_minus_legacy": comparison(
            rows_by[recipes[1][0]], rows_by[recipes[0][0]]
        ),
    ...
    common = {
        fold: set(
            read(dst / f"cv/{selected}_fold{fold}.model.json")["candidate_vocabulary"]
        )
        & set(
            old_e.baseline_bundle(read(dst / f"cv/{selected}_fold{fold}.model.json"))[
                "vocabulary"
            ]
        )
        for fold in range(3)
    ...
    old_hist = [
        enrich(old_t.evaluate_record(r, oldfull), r, old_s, oldfull) for r in historic
    ]
    ...
    for r, row in zip(historic, hist[selected]):
        for br in old_e.baselines(r, row, full_bundles[selected], old=old_m1):
            br["episode"] = row["episode"]
            hist_bases[br["model"]].append(br)
    common_hist = set(full_bundles[selected]["candidate_vocabulary"]) & set(
        old_m1["vocabulary"]
    )
    histmatched = {
        k: [scope_row(r, common_hist, "fine") for r in v]
        for k, v in {**hist, **hist_bases, "M2_JOINT": old_hist}.items()
    }
    save(dst / "cv/historical_baselines_r1.private.json", dict(hist_bases))
    metrics["repair"] = {
        "models": {k: compact_summary(v) for k, v in rows_by.items()},
        "comparisons": comparisons,
        "selected_model": selected,
        "selected_C": selected_recipe[3],
        "matched_fine": {
            selected: compact_summary(selected_fine),
            "B2": compact_summary(b2fine),
        },
        "broad": {
            k: compact_summary([scope_row(r, kind="broad") for r in v])
        ...
        "historical_matched_fine": {
            k: compact_summary(v) for k, v in histmatched.items()
        },
        "historical_comparisons": {
            k: comparison(histmatched[selected], histmatched[k])
            for k in ["B2", "M1", "M2_JOINT"]
        },
        "independent_product_evaluation": "NOT_EVALUATED",
        "selection_scope": "Grouped development CV, reused for selection; historical 17 groups never tune.",
        ...
        "fixed_model_path": str(dst / f"models/{selected}.model.json"),
        "fit_count": len(fitlog),
    }
    save(OUT / "metrics.json", metrics)
    save(
        dst / "selection.private.json",
        {
            "model": selected,
            "C": selected_recipe[3],
            "recipe": selected_recipe,
            "development_score": mean(rows_by[selected], "ndcg5"),
            "model_file": str(dst / f"models/{selected}.model.json"),
        },
    )
    print(
        json.dumps(
...
def diagnostics(owner):
    ...
    dst = owner / "revisions/r1"
    metrics = read(OUT / "metrics.json")
    selection = read(dst / "selection.private.json")
    label = selection["model"]
    source_rows = read(dst / f"cv/{label}.private.json")
    ...
    for fold in range(3):
        ...
        for row in held:
            ...
            for i, st in enumerate(states):
                ...
                stages.append(
                    {
                        **key,
                        ...
                        "delta": (
                            out["ndcg5"] - previous
                            if previous is not None and out["ndcg5"] is not None
                            else None
                        ),
                        "question_count": i,
                        "option_budget": sum(
                            len(a["shown_option_ids"]) for a in answers[:i]
                        ),
                    }
                )
                previous = out["ndcg5"]
                if i:
                    a = answers[i - 1]
                    before = np.asarray(states[i - 1]["features"])
                    after = np.asarray(st["features"])
                    responses.append(
                        {
                            **key,
                            "slot": slot,
                            "selected": "|".join(a["selected_option_ids"])
                            or a["state"],
                            "has_selection": bool(a["selected_option_ids"]),
                            "feature_changed": bool(not np.allclose(before, after)),
                            "score_changed": bool(
                                any(
                                    abs(x["score"] - y["score"]) > 1e-10
                                    for x, y in zip(
                                        sorted(
                                            st["candidate_scores"],
            ...
            q1only = copy.deepcopy(states[2])
            del q1only["answers_by_question"]["Q0"]
            q1only = rt.recompute(q1only, b)
            ...
            if answers:
                ...
                replay_checks.append(
                    np.allclose(full["features"], replayed["features"])
                    and [r["score"] for r in full["candidate_scores"]]
                    == [r["score"] for r in replayed["candidate_scores"]]
                )
            if not ex or not ex["eligible_for_final_comparison"]:
                continue
            direct = set(rt.evidence(full, b)["confirmed"])
            broad = set(rt.evidence(full, b)["broad"])
            selected = [c for c in ex["candidate_ids"] if c in ep["visible"]]
            selected_groups = {
                "REPEATED_EXPLICIT": [c for c in selected if c in direct],
                "BROAD_TO_SPECIFIC": [
                    c
                    for c in selected
                    if c not in direct
                    and bool(set(b["candidate_attributes"].get(c, [])) & broad)
                ],
                "NEW_VISIBLE_JUDGMENT": [
                    c
                    for c in selected
                    if c not in direct
                    and not set(b["candidate_attributes"].get(c, [])) & broad
                ],
            }
                    ...
                    feedback.append(
                        {
                            **key,
                            ...
                            and row["ndcg5"] < 0.5,
                            "exposure_size": len(ex["candidate_ids"]),
                            "generation_version": b["bundle_id"],
    ...
    save(dst / "stage_diagnostics.private.json", stages)
    save(dst / "response_patterns.private.json", responses)
    save(dst / "feedback_diagnostics.private.json", feedback)
    stages_summary = {
        slot: {
            "records": sum(x["slot"] == slot for x in stages),
    ...
    response_summary = {
        slot: {
            "effective_selection_rate": mean(
                [x for x in responses if x["slot"] == slot], "has_selection"
            ),
            "feature_change_rate": mean(
                [x for x in responses if x["slot"] == slot], "feature_changed"
            ),
            "score_change_rate": mean(
                [x for x in responses if x["slot"] == slot], "score_changed"
            ),
            "patterns": dict(
                Counter(x["selected"] for x in responses if x["slot"] == slot)
            ),
        }
    ...
    conditional = tr.entropy([(q0[i], q1[i]) for i in ids]) - tr.entropy(
        [q0[i] for i in ids]
    )
    fb_summary = {}
    for evidence_type in sorted({x["feedback_type"] for x in feedback}):
        for subset in ["ALL", "LARGE_DISCREPANCY_PROXY"]:
            rr = [
                r
                for r in feedback
                if r["feedback_type"] == evidence_type
                and r["selected_count"]
                and (subset == "ALL" or r["large_discrepancy_proxy"])
            ]
            bymode = {
                mode: [r for r in rr if r["model"] == mode]
                for mode in ["F0", "F1", "F2"]
            }
            fb_summary[evidence_type + ":" + subset] = {
                "models": {mode: compact_summary(v) for mode, v in bymode.items()},
                "F2_minus_F0": comparison(bymode["F2"], bymode["F0"]),
                "F2_minus_F1": comparison(bymode["F2"], bymode["F1"]),
                "score_change_max_F2": max(
                    (r["score_delta_max"] for r in bymode["F2"]), default=None
                ),
            }
    metrics["question_diagnostics"] = {
        "prefixes": stages_summary,
    ...
    metrics["feedback_r1"] = {
        ...
        "exposure_and_generation_saved": True,
        "targets_fixed": True,
        "duplicate_replay_cases": len(replay_checks),
        "duplicate_replay_all_zero_change": all(replay_checks),
    }
    save(OUT / "metrics.json", metrics)
    if not all(replay_checks):
        raise AssertionError("DUPLICATE_EVIDENCE_GAIN")
    print(
        json.dumps(
            {
                "phase": "diagnostics",
                "Q1_conditional_bits": conditional,
                "replay_zero": all(replay_checks),
            }
        ),
        flush=True,
    )
...
def main():
    ...
    p.add_argument(
        "--phase",
        choices=["freeze", "reproduce", "repair", "diagnostics"],
        required=True,
    )
    a = p.parse_args()
