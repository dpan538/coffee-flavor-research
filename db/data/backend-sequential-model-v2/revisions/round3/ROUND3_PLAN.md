...
**Status:** ACTIVE — **design pivot 2026-09-12 (R3-D5)**: product line is now `docs/product/FLAVOR_VECTOR_DESIGN_V1.md` (vector space + cosine); the adaptive-question line is archived. Data work continues on the 83K checkpoint. R3-D6 (same day): 12 dimensions, profile-first candidate path, CoffeeReview structure scores — applied in `db/data/product-vector-v1/`. R3-D7 (confirmation): weight 0.6, sources approved, Steps 1–4 executed the same day (Matrix_K, C2 labels, runtime engine); R3-D8: 16 profile names as anchors, α = 0.5, literature intake dir, bilingual presentation layer (tag rules + unified statement schema) — built and tested. R3-D9: coherence decision tree + 8-pick-5 + Q6 gate built in flow.ts (17 tests); consumer lexicon seeded. Step 5 = expand the consumer lexicon from GACTT / social corpora; R3-D10: thresholds 0.80/0.65 and slot mapping approved; flow restructured to check Q2-Q3 together; measured rhythm 25/62/14 (Path 1/2/3+4) locked by tests. R3-D11: 94 tags approved; hybrid utterance mapper + question bank; polarity paradox guard + context check (rhythm 21/52/27 with guard); 37 tests in three suites; literature templates. R3-D12: Q0 wording localised, question bank approved; Step 5 round one — GACTT consumer terms (61 en) + owner fermented zh terms, session.ts front-end chain; 43 tests. R3-D13: GACTT zh mapping, blends (max 3) + optional origin bias, view models / Dexie user library / About content as interfaces (no visuals yet); 51 tests; stack decided (R3-D14): Vue 3 + Tailwind v3 isolated under apps/pwa, 7 components bound to screenModel, PWA build passes; 11 literature claims ingested as PENDING_LOCATOR with two-level declaration; stopped before visual design. R3-D15: visual pass one (tonality, 1:4 flat layout, card fold flow, consumer-facing copy, About spectrum visual, mobile UX details) — fonts and PNG share pending. Remaining CR items are re-scoped by the design doc §10. Phase 0 decided 2026-09-11 (R3-D1..D3 in `owner_decisions_round3.json`): restricted root approved; posture is per-dataset owner approval after manual review; option A recorded. Phase 2 has begun: CoffeeReview parser v1 ingested (`coffeereview_ingestion_finding.json`).
...
- CoffeeReview: **INGESTED v1** — `acquire-coffeereview-round3.py`, 8,387 reviews → 6,180 with a descriptor sentence → 27,011 atoms; 4,670 records carry ≥2 registry words (26x the licensed corpus). **CR-1 DONE (clean stage)**: 77K checkpoint opened on the 40K→50K pattern — 77,038 source assertions, 81,175 atoms, 4,670 records with ≥2 registry words now in the cleaned view; `clean77k` subcommand, 14th route, 4th adapter, owner-review branch in `purpose_rights`. Gates updated loudly (14/4, EXPECTED_FILES, receipt 98→102), two silent-drop gates caught (`stale_gate_audit_round3.json`). B4 left unchanged — correct for its scope. **CR-1b DONE**: semantic layer rebuilt on 77K (`--checkpoint 77k` default; cached public-mode fallback) — edges 121,040→136,065, CONSOLIDATION_V2 3,215→5,050 with 1,835 new CoffeeReview clusters (`cr1b_semantic_77k_finding.json`). Finding: the owner-facing ONTOLOGY_GAP_REGISTER / CONCEPT_CLUSTER_V2 are still cut from 30K/40K by B4/batch6-main — **CR-1b-ii** re-derives the review surface from the 77K consolidation. Downstream: product-inference v0/v0.2 lineage hashes refreshed (outputs byte-identical) — and that identity exposed **F19**: the product generator's semantic join is dead (governed semantic support = 0 for all candidates at 50K and 77K). Repair measured (`f19_product_semantic_join_measurement.json`): 24/120 cases re-rank, 0 state changes, owner packet untouched — owner approved 2026-09-12 and **applied** (guard + positive/negative control in test-product-inference-v0). **F19 repaired** (0865338). **CR-2 DONE**: parser v2 (`cr2_parser_v2_finding.json`) +5,993 assertions / +1,120 reviews, 60% from 2004-2011; new frozen checkpoint **83K** (83,031) with the semantic layer rebuilt on it (141,460 edges). Remaining: CR-1b-ii on 83K, CR-1c.
