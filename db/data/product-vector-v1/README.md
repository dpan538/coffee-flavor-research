...
| `product-vector-v1.json` | Runtime bundle consumed by `packages/flavor-data/src/product-vector-v1`: dimensions, projection, Matrix_K, Matrix_Q, named profiles, alpha, weights, and the `presentation` block (tag lists, concept tags, context statements, tag rules). |
