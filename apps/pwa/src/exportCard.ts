...
const DISPLAY = '"Stack Sans", "MiSans", "Helvetica Neue", system-ui, sans-serif';
...
function tile(ctx: CanvasRenderingContext2D, i: number, x: number, y: number, s: number, color: string) {
  ctx.save();
  ctx.translate(x, y);
  ctx.scale(s, s);
  ctx.fillStyle = color;
...
function wrapWords(ctx: CanvasRenderingContext2D, words: string[], maxWidth: number, gap: number): string[][] {
  const lines: string[][] = [];
  let line: string[] = [];
  let width = 0;
  for (const w of words) {
    ...
    if (line.length && next > maxWidth) { lines.push(line); line = [w]; width = ww; }
    else { line.push(w); width = next; }
  }
  if (line.length) lines.push(line);
  return lines;
}

export async function renderCardPng(model: ResultCardModel, locale: "zh-CN" | "en"): Promise<Blob> {
  const zh = locale === "zh-CN";
  if (typeof document !== "undefined" && document.fonts) {
    await Promise.all([`600 96px ${SANS}`, `500 34px ${SANS}`, `400 30px ${SANS}`, `700 44px ${DISPLAY}`].map((f) => document.fonts.load(f).catch(() => undefined)));
  }
  const S = 1200;
  const canvas = document.createElement("canvas");
  canvas.width = S; canvas.height = S;
  const ctx = canvas.getContext("2d")!;
  ctx.fillStyle = "#1F3B5C"; ctx.fillRect(0, 0, S, S);
  const M = 72, X = M, Y = M, W = S - 2 * M, H = S - 2 * M;
  ctx.fillStyle = "#F3EEE2"; roundRect(ctx, X, Y, W, H, 56); ctx.fill();
  const px = X + 72, pr = X + W - 72;
  let y = Y + 88;
  // eyebrow + motif row
  ctx.fillStyle = "#6B6660"; ctx.font = `500 26px ${SANS}`; ctx.textBaseline = "alphabetic";
  ctx.fillText((zh ? "风味卡" : "FLAVOR CARD").split("").join(zh ? "  " : " "), px, y);
  const colors = model.picked.map((_, i) => DIM_COLORS[model.pickedDimensions[i] ?? ""] ?? FALLBACK[i % FALLBACK.length]!);
  const tileSize = 64, tileGap = 10;
  colors.forEach((c, i) => tile(ctx, i, pr - colors.length * (tileSize + tileGap) + i * (tileSize + tileGap), y - 46, tileSize / 30, c));
  y += 80;
  // cup rows with dotted leaders
  ctx.font = `400 30px ${SANS}`;
  for (const row of model.cupInfo) {
    ctx.fillStyle = "#6B6660"; ctx.fillText(row.label, px, y);
    ctx.fillStyle = "#1E1C1A"; ctx.font = `500 32px ${SANS}`;
    const vw = ctx.measureText(row.value).width;
    ctx.fillText(row.value, pr - vw, y);
    ctx.font = `400 30px ${SANS}`;
    const lw = ctx.measureText(row.label).width;
    ctx.save(); ctx.strokeStyle = "rgba(30,28,26,0.4)"; ctx.lineWidth = 2; ctx.setLineDash([2, 6]);
  ...
  for (const line of lines) {
    let x = px;
    for (const w of line) { ctx.fillText(w, x, y + 80); x += ctx.measureText(w).width + 48; }
    y += 122;
  }
  // rule, reference, signature
  const footY = Y + H - 96;
  ctx.strokeStyle = "rgba(30,28,26,0.15)"; ctx.lineWidth = 2; ctx.setLineDash([]);
  ctx.beginPath(); ctx.moveTo(px, footY - 48); ctx.lineTo(pr, footY - 48); ctx.stroke();
  ctx.fillStyle = "#6B6660"; ctx.font = `400 28px ${SANS}`;
  ctx.fillText(`${zh ? "参考风味" : "Reference"} · ${model.title}`, px, footY);
  ctx.fillStyle = "#1E1C1A"; ctx.font = `700 40px ${DISPLAY}`;
...
export async function shareCardPng(model: ResultCardModel, locale: "zh-CN" | "en"): Promise<"shared" | "saved"> {
  ...
  a.href = url; a.download = "flavorwords-card.png"; a.rel = "noopener";
