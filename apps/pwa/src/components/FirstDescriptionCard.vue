  ...
  <section class="h-full flex flex-col" data-component="FirstDescriptionCard">
    ...
    <div class="flex-1 min-h-0 px-4 pt-5 pb-4 flex flex-col gap-3">
      ...
      <div class="flex flex-wrap gap-2" role="group">
        <button v-for="word in all" :key="word.text" type="button" class="chip" :aria-pressed="chosen.some((w) => w.text === word.text)" :data-dimension="word.dimension" @click="toggle(word)">{{ word.text }}</button>
      </div>
