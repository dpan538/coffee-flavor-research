...
const DIM_COLORS: Record<string, string> = {
  ...
  floral: "#9C92E8",
  fruity: "#EE8F70",
  nutty_chocolate: "#C98A55",
  fermented_winey: "#B2626A",
  bitter_roasted: "#D8D2C8",
  spice: "#DA8A80",
  herbal_green: "#8CC46F",
  woody_earthy: "#4FA06C",
  defect: "#8FA0C8",
...
const FAMILY_LABEL: Record<string, Record<string, string>> = {
  coffeereview_kaggle_parsed: {
    "zh-CN": "CoffeeReview 编辑评审",
    en: "CoffeeReview reviews",
  },
  ace_cup_of_excellence: {
    "zh-CN": "Cup of Excellence 评审",
    en: "Cup of Excellence",
  },
  zenodo_golovinsky_q_grader_dataset: {
    "zh-CN": "Q-grader 数据集",
    en: "Q-grader dataset",
  },
  frontiers_inera_robusta_q_grader_panel: {
    "zh-CN": "罗布斯塔 Q-grader 评审",
    en: "Robusta Q-grader",
  },
  project_origin: {
    "zh-CN": "Project Origin 评审",
    en: "Project Origin panel",
  },
  frontiers_cenicafe_lengupa_trained_cuppers: {
    "zh-CN": "Cenicafé 受训杯测师",
    en: "Cenicafé cuppers",
  },
  coffee_board_of_india_fine_cup: {
    "zh-CN": "印度咖啡局 Fine Cup",
    en: "Coffee Board of India",
  },
  sheba_coffee_yemen_auction: {
    "zh-CN": "也门 Sheba 拍卖评审",
    en: "Sheba Yemen auction",
  },
};
type Family = { family: string; coffees: number; mass: number[] };
const dims = productVectorBundle.dimensions as string[];
const labels = productVectorBundle.presentation.dimension_labels as Record<
  string,
  Record<string, string>
>;
const facts = productVectorBundle.corpus_facts as unknown as {
  families: Family[];
  usable_coffee_vectors: number;
  profiles: number;
};
const families = computed(() =>
  [...facts.families].sort((a, b) => b.coffees - a.coffees),
);
const zh = computed(() => locale.value === "zh-CN");
const fmt = (v: number) =>
  Math.round(v).toLocaleString(locale.value === "zh-CN" ? "zh-CN" : "en-US");
...
const totals = computed(() =>
  dims.map((_, i) => families.value.reduce((a, f) => a + (f.mass[i] ?? 0), 0)),
);
const maxTotal = computed(() => Math.max(...totals.value));
const yOf = (m: number) =>
  BASE -
  (Math.log10(m + 1) / Math.log10(maxTotal.value + 1)) * (BASE - TOP - 14);
const tiers = computed(() =>
  [10, 100, 1000]
    .filter((t) => t < maxTotal.value)
    .map((t) => ({ t, y: yOf(t) })),
);
const minorTiers = computed(() =>
  [3, 30, 300, 3000]
    .filter((t) => t < maxTotal.value)
    .map((t) => ({ t, y: yOf(t) })),
);
...
const share = (i: number) =>
  `${Math.round((totals.value[i]! / grand.value) * 100)}%`;
...
<template>
  <figure class="w-full text-[#F4F1EA]" data-component="SourceColumns">
    ...
    >
      <div
        class="text-[13px] leading-[1.45] text-[#B9B4AA]"
        :class="zh ? 'max-w-[190px]' : ''"
      >
        {{
          zh
            ? "十二类风味特征在全部评审记录里的累计权重（对数刻度）；深浅是来源评审族，字母下是各自占比。"
            : "Twelve flavor features by cumulative weight across all review records (log scale); shades are review panels, shares under each letter."
        }}
      </div>
      <ul
        class="text-[12px] leading-[1.4] shrink-0"
        :class="zh ? '' : 'grid grid-cols-2 gap-x-3'"
        data-index="panels"
      >
        <li
          ...
          :style="{ opacity: panelIn(i) }"
        >
          <span
            class="inline-block w-3 h-[8px] mt-[4px] rounded-[1px] bg-[#F4F1EA]"
            :style="{ opacity: i === 0 ? 1 : i === 1 ? 0.7 : 0.42 }"
          /><span class="text-[#B9B4AA]"
            >{{ FAMILY_LABEL[f.family]?.[locale] ?? f.family }}
            <b class="text-[#F4F1EA] font-medium tabular-nums">{{
              fmt(f.coffees)
            }}</b></span
          >
        </li>
    ...
    <ul
      class="grid grid-cols-3 gap-x-2 gap-y-[4px] mt-3 text-[13px] leading-[1.3]"
      data-index="features"
      :style="{ opacity: axesIn }"
    >
      <li v-for="(d, i) in dims" :key="d" class="flex items-center gap-1.5">
        <b class="w-3 font-semibold">{{ LETTERS[i] }}</b
        ><span
          class="inline-block w-2 h-2 rounded-sm"
          :style="{ backgroundColor: DIM_COLORS[d] }"
