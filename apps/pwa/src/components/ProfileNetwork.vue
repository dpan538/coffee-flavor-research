...
const W = 375, H = 470, CX = 187.5, CY = 235, R1 = 78, R2 = 148;
...
<template>
  <figure class="w-full text-[#F4F1EA]" data-component="ProfileNetwork">
    <div class="flex items-start justify-between gap-3">
      <p class="text-[10.5px] leading-[1.35] text-[#B9B4AA] max-w-[200px]">{{ zh ? '圆圈是本项目整理的 16 组参考风味：大小按记录数，颜色是该组最强的风味特征；内圈是记录最多的 6 组。白线把每组连到与它最接近的两组，C1–C6 标出最接近的几对。' : 'Circles are the 16 reference profiles this project organised: size by records, colour by the group\'s strongest feature; the inner ring holds the 6 largest. White curves join each group to its two nearest, C1–C6 mark the closest pairs.' }}</p>
      <ol class="grid grid-cols-2 gap-x-3 gap-y-[1px] text-[9.5px] leading-[1.3] shrink-0" data-index="profiles">
        <li v-for="(nd, i) in nodes" :key="nd.id" class="flex gap-1 transition-opacity duration-300" :style="{ opacity: nodeIn(i) }"><b class="w-3 text-right font-semibold tabular-nums">{{ nd.n }}</b><span class="text-[#D8D2C8]">{{ nd.name }}</span></li>
      </ol>
    </div>
    <svg :viewBox="`0 0 ${W} ${H}`" class="w-full h-auto mt-1" role="img">
      <g :opacity="ringsIn">
        ...
        <text :x="CX" :y="CY + R1 + 17" text-anchor="middle" font-size="8.5" fill="#B9B4AA">{{ zh ? '记录最多的 6 组' : 'the 6 largest' }}</text>
        <text :x="CX" :y="CY - R2 - 5" text-anchor="middle" font-size="8.5" fill="#B9B4AA">{{ zh ? '其余 10 组' : 'the other 10' }}</text>
        <circle :cx="CX" :cy="CY" r="26" fill="#F4F1EA" />
        <text :x="CX" :y="CY - 2" text-anchor="middle" font-size="9.5" font-weight="700" fill="#0B0A09">{{ fmt(nodes.reduce((a, n) => a + n.members, 0)) }}</text>
        <text :x="CX" :y="CY + 9" text-anchor="middle" font-size="7.5" fill="#4a4642">{{ zh ? '条记录' : 'records' }}</text>
      </g>
      ...
      <g v-for="e in callouts" :key="e.callout" :opacity="notesIn">
        <rect :x="e.mx - 11" :y="e.my - 7" width="22" height="14" rx="2" fill="#F4F1EA" />
        <text :x="e.mx" :y="e.my + 3.5" text-anchor="middle" font-size="8.5" font-weight="700" fill="#0B0A09">{{ e.callout }}</text>
      </g>
    </svg>
    <div class="grid grid-cols-2 gap-x-3 mt-1 transition-opacity duration-500" :style="{ opacity: notesIn }">
      <ul class="text-[9.5px] leading-[1.35] text-[#D8D2C8]" data-index="callouts">
        <li v-for="e in callouts" :key="e.callout"><b class="text-[#F4F1EA]">{{ e.callout }}</b> {{ e.a.n }} ↔ {{ e.b.n }} · {{ zh ? '共同点' : 'shared' }}：{{ e.shared.join("、") }} <span class="text-[#8f8a80] tabular-nums">{{ e.sim.toFixed(2) }}</span></li>
      </ul>
      <ul class="grid grid-cols-2 gap-x-2 gap-y-[1px] text-[9.5px] leading-[1.35]" data-index="features">
        <li v-for="d in dims" :key="d" class="flex items-center gap-1"><span class="inline-block w-2 h-2 rounded-sm" :style="{ backgroundColor: DIM_COLORS[d] }" /><span class="text-[#D8D2C8]">{{ labels[d]?.[locale] }}</span></li>
      </ul>
    </div>
  </figure>
</template>
