...
<template>
  <figure class="w-full text-[#F4F1EA]" data-component="ProfileNetwork">
    <div class="flex flex-col gap-2">
      ...
      <ol class="grid grid-cols-2 gap-x-3 gap-y-[2px] leading-[1.25]" :class="zh ? 'text-[12px]' : 'text-[11px]'" data-index="profiles">
        <li v-for="(nd, i) in nodes" :key="nd.id" class="flex gap-1.5 min-w-0 transition-opacity duration-300" :style="{ opacity: nodeIn(i) }"><b class="w-4 text-right font-semibold tabular-nums shrink-0">{{ nd.n }}</b><span class="text-[#D8D2C8]">{{ nd.name }}</span></li>
      </ol>
    </div>
    <svg :viewBox="`0 0 ${W} ${H}`" class="w-full h-auto mx-auto mt-1" :class="zh ? 'max-h-[35dvh]' : 'max-h-[31dvh]'" role="img">
      <g :opacity="ringsIn">
    ...
    <div class="mt-2 transition-opacity duration-500" :style="{ opacity: notesIn }">
      <ul class="grid grid-cols-2 gap-x-3 gap-y-[2px] leading-[1.3] text-[#D8D2C8]" :class="zh ? 'text-[11.5px]' : 'text-[11px]'" data-index="callouts">
      ...
      <ul class="grid gap-x-2 gap-y-[3px] mt-2 leading-[1.25]" :class="zh ? 'grid-cols-3 text-[12px]' : 'grid-cols-4 text-[11px]'" data-index="features">
