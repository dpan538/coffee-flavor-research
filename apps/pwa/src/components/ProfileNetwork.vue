...
const fmt = (v: number) =>
  v.toLocaleString(locale.value === "zh-CN" ? "zh-CN" : "en-US");
const W = 375,
  H = 372,
  CX = 187.5,
  CY = 186,
...
const nodes = computed<Node[]>(() => {
  const sorted = [...productVectorBundle.profiles].sort(
  ...
  return sorted.map((p, i) => {
    ...
    const angle =
      -Math.PI / 2 +
      (inner ? Math.PI / 6 : Math.PI / count) +
      (k / count) * Math.PI * 2;
    const R = inner ? R1 : R2;
    const top = dims
      .map((d, j) => ({ d, w: p.centroid[j] ?? 0 }))
      .sort((a, b) => b.w - a.w)
      .slice(0, 2)
      .map((x) => x.d);
    return {
      n: i + 1,
      id: p.profile_id,
      name: p.owner_name[locale.value] || p.owner_name.en,
      members: p.member_count,
      x: CX + R * Math.cos(angle),
      y: CY + R * Math.sin(angle),
      r: 11 + 11 * (Math.log10(p.member_count + 1) / max),
      color: DIM_COLORS[top[0]!] ?? "#ccc",
      top,
      centroid: p.centroid as number[],
    };
  });
});
...
const edges = computed<Edge[]>(() => {
  ...
  for (const a of ns) {
    const nearest = ns
      .filter((b) => b !== a)
      .map((b) => ({ b, sim: cosine(a.centroid, b.centroid) }))
    ...
    for (const { b, sim } of nearest) {
      ...
      const shared = dims
        .map((d, i) => ({ d, w: mean[i] ?? 0 }))
        .sort((x, y) => y.w - x.w)
        .slice(0, 2)
        .map((x) => labels[x.d]?.[locale.value] ?? x.d);
  ...
  <figure class="w-full text-[#F4F1EA]" data-component="ProfileNetwork">
    <div class="flex flex-col gap-2">
      <p class="text-[13px] leading-[1.45] text-[#B9B4AA]">
        {{
          zh
            ? "大小按记录数，颜色是该组最强的风味特征；内圈是记录最多的 6 组。白线连到最接近的两组，C1–C6 是最接近的几对。"
            : "Size by records, colour by the group's strongest feature; the inner ring holds the 6 largest. Curves join the two nearest groups, C1–C6 the closest pairs."
        }}
      </p>
      <ol
        class="grid grid-cols-2 gap-x-3 gap-y-[2px] leading-[1.25]"
        :class="zh ? 'text-[12px]' : 'text-[11px]'"
        data-index="profiles"
      >
        <li
          v-for="(nd, i) in nodes"
          :key="nd.id"
          class="flex gap-1.5 min-w-0 transition-opacity duration-300"
          :style="{ opacity: nodeIn(i) }"
        >
          <b class="w-4 text-right font-semibold tabular-nums shrink-0">{{
            nd.n
          }}</b
          ><span class="text-[#D8D2C8]">{{ nd.name }}</span>
        </li>
      </ol>
    </div>
    <svg
    ...
    >
      <ul
        class="grid grid-cols-2 gap-x-3 gap-y-[2px] leading-[1.3] text-[#D8D2C8]"
        :class="zh ? 'text-[11.5px]' : 'text-[11px]'"
        data-index="callouts"
      >
        <li v-for="e in callouts" :key="e.callout">
          <b class="text-[#F4F1EA]">{{ e.callout }}</b> {{ e.a.n }} ↔
          {{ e.b.n }} · {{ e.shared.join("、") }}
          <span class="text-[#8f8a80] tabular-nums">{{
            e.sim.toFixed(2)
          }}</span>
        </li>
      </ul>
      <ul
        class="grid gap-x-2 gap-y-[3px] mt-2 leading-[1.25]"
        :class="zh ? 'grid-cols-3 text-[12px]' : 'grid-cols-4 text-[11px]'"
        data-index="features"
      >
        <li v-for="d in dims" :key="d" class="flex items-start gap-1">
          <span
            class="inline-block w-2 h-2 rounded-sm shrink-0 mt-[4px]"
