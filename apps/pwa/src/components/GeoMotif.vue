...
const props = withDefaults(
  defineProps<{
    variant?: "card" | "home" | "about";
    colors?: string[];
    size?: number;
  }>(),
  { variant: "card", colors: () => [], size: 24 },
);
...
const layout = computed<{ cols: number; rows: number; tiles: Tile[] }>(() => {
  ...
  if (props.variant === "about") {
    return {
      ...
      rows: 1,
      tiles: [
        { fg: "#B97C4E", shape: "diamond" },
        { fg: "#7268C9", shape: "quarter" },
        { fg: "#6F8B5A", shape: "circle" },
        { fg: "#E4724B", shape: "half" },
        { fg: "#1F3B5C", shape: "triangle" },
      ],
    };
  }
...
const width = computed(
  () => layout.value.cols * 30 + (layout.value.cols - 1) * GAP,
);
const height = computed(
  () => layout.value.rows * 30 + (layout.value.rows - 1) * GAP,
);
</script>
...
<template>
  <svg
    ...
    :width="(width / 30) * size"
    :height="(height / 30) * size"
    aria-hidden="true"
    data-component="GeoMotif"
    :data-variant="variant"
  >
    <g
      v-for="(t, i) in layout.tiles"
      :key="i"
