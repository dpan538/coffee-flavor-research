...
<template>
  <article class="rounded-[22px] bg-cream text-ink" :class="compact ? 'px-4 pt-4 pb-3' : 'px-5 pt-5 pb-4'" data-component="FlavorCard">
    <div class="flex items-center justify-between">
      ...
      <GeoMotif class="ml-auto" variant="card" :size="compact ? 20 : 24" :colors="colors" />
    </div>
    ...
    <p class="flex flex-wrap gap-x-4 gap-y-0 font-semibold" :class="compact ? 'text-[22px] leading-[1.3] mt-5' : 'text-[27px] leading-[1.3] mt-9'" data-layer="words">
      <span v-for="w in words" :key="w">{{ w }}</span>
    </p>
    <div class="flex items-end justify-between mt-7 border-t border-ink/15 pt-2" data-layer="reference">
