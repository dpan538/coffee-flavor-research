...
const roleOf = (id: string) =>
  citations.value.find((c) => c.id === id)?.role ?? "";
function toTop() {
...
onMounted(async () => {
  ...
  ScrollTrigger.create({
    trigger: flowTrack.value!,
    scroller: scrollerEl,
    start: "top top",
    end: "bottom bottom",
    scrub: 0.6,
    onUpdate: (self) => (flowProgress.value = self.progress),
  });
  ScrollTrigger.create({
    trigger: burstTrack.value!,
    scroller: scrollerEl,
    start: "top top",
    end: "bottom bottom",
    scrub: 0.6,
    onUpdate: (self) => (burstProgress.value = self.progress),
  });
  ...
  gsap.from(scrollerEl.querySelectorAll("[data-page='1'] > *"), {
    y: 24,
    opacity: 0,
    duration: 0.7,
    ease: "power3.out",
...
<template>
  ...
  >
    ...
    >
      ...
      <p class="text-[11px] tracking-[0.22em] text-muted mt-8">
        {{ approach.eyebrow }}
      </p>
      <h1
        class="text-[36px] mt-2 font-bold"
        :class="zh ? 'display-zh' : 'display'"
      >
        {{ approach.title }}
      </h1>
      <p
        v-for="(p, i) in approach.intro"
        :key="'i' + i"
        class="leading-relaxed text-[16px] mt-4"
      >
        {{ p }}
      </p>
      <p class="text-xs text-muted mt-auto pt-8 tracking-widest">
        ↓ {{ t.scroll }}
      </p>
    </section>
    ...
    <section
      ref="flowTrack"
      class="relative h-[560dvh] bg-[#0B0A09]"
      data-page="3"
      data-section="flow"
    >
      <div
    ...
    >
      ...
      >
        ...
        <h2
          class="text-[32px] mt-2 font-bold"
          :class="zh ? 'display-zh' : 'display'"
        >
          {{ author.title }}
        </h2>
        <p
          v-for="(p, i) in author.paragraphs"
          :key="'a' + i"
          class="leading-relaxed mt-4 text-[16px]"
        >
          {{ p }}
        </p>
        <div class="mt-8 divide-y divide-ink/10 border-y border-ink/10">
          ...
          >
            <button
              type="button"
              class="fold-toggle py-4"
              :aria-expanded="openSection === section.id"
              @click="toggle(section.id)"
            >
              <span>
                <span class="block text-[18px] font-medium">{{
                  section.title
                }}</span>
                <span class="block text-sm text-muted mt-0.5 leading-relaxed">{{
                  section.summary
                }}</span>
              </span>
              <span
                class="text-2xl leading-none ml-4 shrink-0 font-semibold"
                :style="{ color: PLUS_COLORS[si % PLUS_COLORS.length] }"
                >{{ openSection === section.id ? "–" : "+" }}</span
              >
            </button>
              ...
              <div>
                <div class="pb-5 space-y-3">
                  <article
                    ...
                    :key="block.title"
                    class="rounded-2xl bg-cream p-4"
                  >
      ...
      >
        ...
        <ol class="mt-5 relative">
          <li v-for="(e, i) in evidence" :key="e.id" class="relative pl-6 pb-6">
            ...
            />
            <span
              v-if="i < evidence.length - 1"
              class="absolute left-[5px] top-6 bottom-0 w-px bg-ink/15"
            />
            ...
            <p class="text-[13px] text-muted leading-relaxed mt-1">
              <span class="text-ink">{{ t.uses }}：</span>{{ e.use }}
            </p>
            <button
              type="button"
              class="chip mt-3 !min-h-[44px] !py-2 text-sm"
              :aria-expanded="openSection === e.id"
              @click="toggle(e.id)"
            >
              {{ openSection === e.id ? t.collapse : t.details }}
            </button>
            <div class="fold" :data-open="openSection === e.id">
              <div>
                <p class="text-xs text-muted mt-3 break-all">{{ e.locator }}</p>
                <p class="text-xs text-muted mt-1">
                  <span class="text-ink">{{ t.terms }}：</span>{{ e.terms }}
                </p>
              </div>
            </div>
          </li>
