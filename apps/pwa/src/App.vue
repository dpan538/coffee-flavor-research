      ...
      <Transition :css="false" mode="out-in" @enter="onEnter" @leave="onLeave">
        <div :key="stageKey()" class="h-full">
          <section v-if="stage === 'hero'" class="h-full flex items-center gap-3 px-5 py-6" data-screen="hero">
            <button type="button" class="tile flex-1 aspect-[1/1.32] bg-sun text-ink rise" data-action="start" @click="start">
            ...
            <button type="button" class="tile flex-1 aspect-[1/1.32] bg-mint2 text-ink rise" data-action="about" @click="aboutOpen = true">
