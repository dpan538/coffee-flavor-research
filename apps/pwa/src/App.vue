...
const headline = computed(() => {
  ...
  if (at < 0)
    return {
      before: text,
      letters: [] as Array<{ ch: string; color: string }>,
      after: "",
    };
  return {
    ...
    letters: text
      .slice(at, at + 5)
      .split("")
      .map((ch, i) => ({ ch, color: LETTER_COLORS[i]! })),
...
const stageKey = () => {
  ...
  if (stage.value === "context")
    return `context:${currentContextCard.value?.key ?? ""}`;
...
function onEnter(el: Element, done: () => void) {
  ...
  gsap.fromTo(
    el,
    { y: 44, scale: 0.965, opacity: 0, transformOrigin: "50% 100%" },
    {
      y: 0,
      scale: 1,
      opacity: 1,
  ...
  >
    ...
    >
      ...
      <div
        ...
        aria-live="polite"
        :aria-label="locale === 'zh-CN' ? '收集中' : 'collecting'"
      >
        <span class="dot w-2 h-2 rounded-full bg-ink" /><span
          class="dot w-2 h-2 rounded-full bg-ink"
        /><span class="dot w-2 h-2 rounded-full bg-ink" />
      </div>
    </section>
